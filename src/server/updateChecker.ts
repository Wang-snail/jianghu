...
function fetchJson(url: string): Promise<unknown> {
  return new Promise((resolve, reject) => {
    const req = https.get(url, { headers: { 'User-Agent': 'company-update-checker' } }, (res) => {
      const chunks: Buffer[] = []
...
export async function forceCheck(options: Pick<UpdateCheckerOptions, 'onReadyUpdate'> = {}): Promise<void> {
  try {
    const releases = await fetchJson(
      'https://api.github.com/repos/company-ai/room/releases?per_page=100'
    ) as GithubRelease[]
    ...
    for (const a of latest.assets) {
      ...
      else if (name.startsWith('company-update-') && name.endsWith('.tar.gz')) updateBundle = url
    }
...
export async function simulateUpdate(): Promise<void> {
  ...
  cached = {
    ...
    releaseUrl: 'https://github.com/company-ai/room/releases',
    assets: {
