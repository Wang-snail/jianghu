...
const DEFAULT_CLOUD_ALLOWED_ORIGINS = ['http://127.0.0.1:4700', 'http://localhost:4700']
...
function getCloudAllowedOrigins(): Set<string> {
  const configured = (process.env.COMPANY_ALLOWED_ORIGINS || '')
    .split(',')
...
function getCloudJwtSecret(): string {
  return (process.env.COMPANY_CLOUD_JWT_SECRET || '').trim()
}

function getCloudInstanceId(): string {
  return (process.env.COMPANY_CLOUD_INSTANCE_ID || '').trim()
}
...
function validateCloudJwt(token: string): { role: 'user' | 'member'; profile: CloudTokenProfile } | null {
  ...
  if (payload.iss !== 'company-cloud' || payload.aud !== 'company-runtime') return null
...
export function getDeploymentMode(): DeploymentMode {
  return normalizeDeploymentMode(process.env.COMPANY_DEPLOYMENT_MODE)
}
