...
export type ClerkApiProvider = 'openai_api' | 'anthropic_api' | 'gemini_api' | 'mimo_api'
...
function findAnyRoomCredential(db: Database.Database, credentialName: 'openai_api_key' | 'anthropic_api_key' | 'gemini_api_key' | 'mimo_api_key'): string | null {
...
function readProviderEnvKey(provider: ClerkApiProvider, primaryEnvVar: string): string | null {
  const primary = (process.env[primaryEnvVar] || '').trim()
...
function getClerkApiAuthState(db: Database.Database, provider: ClerkApiProvider): ClerkApiAuthState {
  const credentialName = provider === 'openai_api' ? 'openai_api_key' : provider === 'gemini_api' ? 'gemini_api_key' : provider === 'mimo_api' ? 'mimo_api_key' : 'anthropic_api_key'
  const envVar = provider === 'openai_api' ? 'OPENAI_API_KEY' : provider === 'gemini_api' ? 'GEMINI_API_KEY' : provider === 'mimo_api' ? 'MIMO_API_KEY' : 'ANTHROPIC_API_KEY'
  const roomCredential = findAnyRoomCredential(db, credentialName)
  const savedKey = queries.getClerkApiKey(db, provider)
  const envKey = readProviderEnvKey(provider, envVar)
...
export function getClerkApiAuth(db: Database.Database): { openai: ClerkApiAuthState; anthropic: ClerkApiAuthState; gemini: ClerkApiAuthState; mimo: ClerkApiAuthState } {
  return {
    mimo: getClerkApiAuthState(db, 'mimo_api'),
...
export function resolveClerkApiKey(db: Database.Database, model: string | null | undefined): string | undefined {
  ...
  if (provider === 'mimo_api') {
    return findAnyRoomCredential(db, 'mimo_api_key')
      || queries.getClerkApiKey(db, 'mimo_api')
      || (process.env.MIMO_API_KEY || undefined)
  }
  return undefined
}
...
function buildExecutionCandidates(db: Database.Database, preferredModel: string): ClerkExecutionCandidate[] {
  ...
  for (const model of buildClerkModelPlan(preferredModel)) {
    ...
    if ((provider === 'openai_api' || provider === 'anthropic_api' || provider === 'gemini_api' || provider === 'mimo_api') && !apiKey) continue
...
function isProviderUnavailableFailure(message: string): boolean {
  const lower = message.toLowerCase()
  return lower.includes('cli not found')
...
export async function executeClerkWithFallback(options: ClerkExecutionOptions): Promise<ClerkExecutionOutcome> {
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    if (!hasMoreCandidates || (!isTransientFailure(error, result.timedOut) && !isProviderUnavailableFailure(error))) {
      return {
        ok: false,
        model: candidate.model,
        output: '',
