...
function getDefaultDataDir(): string {
  return join(homedir(), '.company-local')
}

export function getDataDir(): string {
  const raw = process.env.COMPANY_DATA_DIR || getDefaultDataDir()
...
export function getServerDatabase(): Database.Database {
  ...
  const rawPath = process.env.COMPANY_DB_PATH || join(dataDir, 'data.db')
  const dbPath = expandTilde(rawPath)
  if (process.env.COMPANY_DEBUG_DB_PATH === '1') {
    console.error(`API server DB path: ${dbPath}`)
  }
