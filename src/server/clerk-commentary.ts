...
/** Map raw MCP/company tool names to human-friendly labels */
const TOOL_NAMES: Record<string, string> = {
  // Memory
  company_remember: 'saving to memory',
  company_recall: 'searching memory',
  company_forget: 'removing memory',
  company_memory_list: 'listing memories',
  // Workers
  company_create_worker: 'creating a worker',
  company_list_workers: 'checking workers',
  company_update_worker: 'updating a worker',
  company_delete_worker: 'removing a worker',
  // Goals
  company_set_goal: 'setting a goal',
  company_complete_goal: 'completing goal',
  company_list_goals: 'checking goals',
  company_delegate_task: 'delegating task',
  // Messages & inbox
  company_send_message: 'sending a message',
  company_inbox_list: 'checking inbox',
  company_inbox_reply: 'replying to message',
  company_inbox_send_room: 'messaging another room',
  company_list_keeper_requests: 'checking keeper requests',
  company_resolve_escalation: 'answering room escalation',
  company_keeper_vote: 'casting keeper vote',
  company_reply_room_message: 'replying to room inbox message',
  // Decisions / quorum
  company_announce: 'announcing a decision',
  company_object: 'objecting to a decision',
  company_list_decisions: 'checking decisions',
  company_decision_detail: 'reading decision details',
  // Tasks & scheduling
  company_list_tasks: 'listing tasks',
  company_run_task: 'running a task',
  company_pause_task: 'pausing a task',
  company_delete_task: 'deleting a task',
  // Web
  company_web_search: 'searching the web',
  company_web_fetch: 'fetching a webpage',
  WebSearch: 'searching the web',
  WebFetch: 'fetching a webpage',
  // Skills
  company_create_skill: 'creating a skill',
  company_list_skills: 'listing skills',
  company_activate_skill: 'activating a skill',
  // Watches
  company_watch: 'setting up a watch',
  company_list_watches: 'checking watches',
  // Wallet
  company_wallet_balance: 'checking wallet balance',
  company_wallet_send: 'sending funds',
  company_wallet_history: 'checking wallet history',
  // Rooms
  company_list_rooms: 'listing rooms',
  company_room_status: 'checking room status',
  // Stations
  company_station_create: 'creating a station',
  company_station_list: 'listing stations',
  company_station_exec: 'running station command',
  // Identity & invites
  company_identity_get: 'checking identity',
  company_invite_create: 'creating an invite',
  company_invite_list: 'listing invites',
  company_invite_network: 'checking network',
  // Credentials
  company_credentials_list: 'listing credentials',
  company_credentials_get: 'getting a credential',
  // Settings
  company_get_setting: 'reading a setting',
  quoorum_set_setting: 'changing a setting',
  // Self-mod
  company_self_mod_edit: 'editing own prompt',
  company_self_mod_history: 'checking edit history',
  // Browser
  company_browser: 'using the browser',
  // Tool search
...
function extractToolName(content: string): string | null {
  // "Step 2: Using mcp__company__company_create_skill" (API model format)
  const stepUsing = content.match(/Using\s+(?:mcp__\w+__)?(\w+)/)
  if (stepUsing) return stepUsing[1]
  // "→ mcp__company__company_create_skill({...})" or "tool_name({...})"
  const withArgs = content.match(/^→?\s*(?:mcp__\w+__)?(\w+)\(/)
  if (withArgs) return withArgs[1]
  // Bare tool name: "mcp__company__company_create_skill" or "company_create_skill"
...
function friendlyName(rawName: string): string {
  const baseName = rawName.replace(/^company_/, '')
  const fullKey = `company_${baseName}`
...
function sanitizeContent(text: string): string {
  ...
  return text.replace(/mcp__\w+__(\w+)/g, (_, name) => friendlyName(name))
    // Also replace bare company_xxx patterns
    .replace(/\bcompany_(\w+)/g, (_, name) => friendlyName(`company_${name}`))
}
