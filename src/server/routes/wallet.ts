...
import type { OnChainBalance, WalletTransactionCategory } from '../../shared/types'
...
export function registerWalletRoutes(router: Router): void {
  ...
  router.post('/api/rooms/:roomId/wallet/transactions', (ctx) => {
    const roomId = Number(ctx.params.roomId)
    const room = queries.getRoom(ctx.db, roomId)
    if (!room) return { status: 404, error: 'Company not found' }
    const wallet = queries.getWalletByRoom(ctx.db, roomId)
    if (!wallet) return { status: 400, error: 'Company has no wallet' }
    ...
    const category = body.category ?? (direction === 'income' ? 'revenue' : 'expense')
    const tx = queries.logInternalBankTransaction(ctx.db, roomId, {
      direction,
  ...
  router.post('/api/rooms/:roomId/wallet/withdraw', async (ctx) => {
    ...
    const encryptionKey = crypto.createHash('sha256')
      .update(`company-wallet-${room.id}-${room.name}`)
      .digest('hex')
