...
function streamWithRedirects(
  ...
  try {
    ...
    mod.get(url, { headers: { 'User-Agent': 'company-updater/1.0' } }, (assetRes) => {
      if (
...
export interface ServerOptions {
  ...
  port?: number
  /** Data directory for token/port files. Defaults to ~/.company */
  dataDir?: string
...
function isPrivateLanAddress(address: string | undefined): boolean {
...
const PROFILE_HTTP = process.env.COMPANY_PROFILE_HTTP === '1'
const PROFILE_HTTP_SLOW_MS = Math.max(1, Number.parseInt(process.env.COMPANY_PROFILE_HTTP_SLOW_MS ?? '300', 10) || 300)
...
function resolveStaticDirForStart(): string | undefined {
  const forcedStaticDir = process.env.COMPANY_STATIC_DIR
  if (forcedStaticDir && fs.existsSync(path.join(forcedStaticDir, 'index.html'))) {
    return forcedStaticDir
  }
...
// ─── Rate limiting (both local and cloud mode) ───────────────────────────
const RATE_LIMIT_WINDOW_MS = 60_000

// Local mode: more relaxed limits (single-user, trusted environment)
const RATE_LIMIT_READ_LOCAL = 1000   // GET requests per minute per IP
const RATE_LIMIT_WRITE_LOCAL = 300   // mutation requests per minute per IP

// Cloud mode: stricter limits (multi-tenant, public-facing)
const RATE_LIMIT_READ_CLOUD = 300   // GET requests per minute per IP
const RATE_LIMIT_WRITE_CLOUD = 120  // mutation requests per minute per IP
...
setInterval(() => {
  ...
  if (cleaned > 0 && process.env.COMPANY_DEBUG_RATE_LIMIT) {
    console.log(`[RateLimit] Cleaned up ${cleaned} expired buckets (${rateBuckets.size} active)`)
  }
}, 120_000).unref()

function checkRateLimit(ip: string, method: string): { allowed: boolean; retryAfter: number; remaining: number; limit: number; resetAt: number } {
  const isCloud = isCloudDeployment()
  const isWrite = method !== 'GET' && method !== 'HEAD' && method !== 'OPTIONS'
  const limit = isWrite
    ? (isCloud ? RATE_LIMIT_WRITE_CLOUD : RATE_LIMIT_WRITE_LOCAL)
    : (isCloud ? RATE_LIMIT_READ_CLOUD : RATE_LIMIT_READ_LOCAL)
  ...
  if (bucket.count > limit) {
    ...
    return { allowed: false, retryAfter, remaining: 0, limit, resetAt: bucket.resetAt }
  }
  return { allowed: true, retryAfter: 0, remaining: limit - bucket.count, limit, resetAt: bucket.resetAt }
}
...
export function createApiServer(options: ServerOptions = {}): {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    // Rate limiting for API routes (both local and cloud mode)
    if (pathname.startsWith('/api/')) {
      const clientIp = req.socket.remoteAddress || 'unknown'
      const rl = checkRateLimit(clientIp, req.method || 'GET')
      // Add rate limit headers to all API responses
      responseHeaders['X-RateLimit-Limit'] = String(rl.limit)
      responseHeaders['X-RateLimit-Remaining'] = String(rl.remaining)
      responseHeaders['X-RateLimit-Reset'] = new Date(rl.resetAt).toISOString()
      if (!rl.allowed) {
    ...
    if (pathname === '/api/auth/handshake' && req.method === 'GET') {
      ...
      const isLanClient = isPrivateLanAddress(req.socket.remoteAddress)
        && isPrivateLanHost(req.headers.host)
        && (isSameOrigin || !origin)
      if ((!isLocalClient || (origin && !isLocalOrigin(origin))) && !isLanClient) {
        res.writeHead(403, handshakeHeaders)
        res.end(JSON.stringify({ error: 'Handshake allowed only from localhost or same-LAN clients' }))
        return
      }
...
/**
 * Patch Codex TOML config to register company MCP server.
...
/**
 * Ensure Claude Code settings auto-approve company MCP tools so headless queen
 * sessions don't get stuck prompting for permission.
 * Patches ~/.claude/settings.json → permissions.allow with "mcp__company__*".
 */
...
function registerMcpGlobally(dbPath: string): void {
  try {
    ...
    const entry = (source: string) => ({
      command: nodePath,
      args: [mcpServerPath],
      env: { COMPANY_DB_PATH: dbPath, COMPANY_SOURCE: source },
    })
...
export function startServer(options: ServerOptions = {}): void {
  ...
  const bindHost = process.env.COMPANY_BIND_HOST
  ...
  // This prevents serving outdated ~/.company/app/ui after a full installer upgrade.
  initBootHealthCheck()
  ...
  const dbPath = process.env.COMPANY_DB_PATH || path.join(options.dataDir ?? getDataDir(), 'data.db')
  ...
  // Skip during tests (COMPANY_SKIP_MCP_REGISTER=1) to avoid clobbering real config.
  if (!process.env.COMPANY_SKIP_MCP_REGISTER) {
    registerMcpGlobally(dbPath)

    // Re-register when a provider is installed or connected — their config files
    // may not have existed at startup (e.g. user installs codex after company).
  ...
  function listen(): void {
    server.listen(port, bindHost, () => {
      ...
      console.error(`江湖 API server started on http://localhost:${boundPort}`)
      ...
      // COMPANY_NO_AUTO_OPEN is set by the macOS tray app which manages browser opening itself.
      if (process.env.NODE_ENV === 'production' && deploymentMode !== 'cloud' && !process.env.COMPANY_NO_AUTO_OPEN) {
