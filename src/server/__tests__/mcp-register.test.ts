 ...
 *
 * We exercise the logic by calling startServer() with COMPANY_SKIP_MCP_REGISTER unset
...
beforeEach(() => {
  tmpDir = join(tmpdir(), `company-mcp-test-${Date.now()}`)
...
describe('patchMcpConfig', () => {
  const entry = { command: '/usr/bin/node', args: ['/path/to/server.js'], env: { COMPANY_DB_PATH: '/tmp/test.db' } }
  ...
  it('injects company into an existing config with other servers', () => {
    ...
    expect(config.mcpServers.company).toEqual(entry)
  ...
  it('overwrites existing company entry with updated values', () => {
    const configPath = join(tmpDir, 'config.json')
    writeFileSync(configPath, JSON.stringify({
      mcpServers: { company: { command: '/old/node', args: ['/old/server.js'] } }
    }))

    const newEntry = { command: '/new/node', args: ['/new/server.js'], env: { COMPANY_DB_PATH: '/new.db' } }
    patchMcpConfig(configPath, newEntry)

    const config = JSON.parse(readFileSync(configPath, 'utf-8'))
    expect(config.mcpServers.company.command).toBe('/new/node')
  })
...
describe('patchCodexConfig', () => {
  const nodePath = '/usr/local/lib/company/runtime/node'
  const serverPath = '/usr/local/lib/company/lib/server.js'
  const dbPath = '/home/user/.company/data.db'
  ...
  it('appends company MCP section to existing config', () => {
    ...
    expect(content).toContain('[mcp_servers.company]')
    expect(content).toContain(`command = '${nodePath}'`)
    expect(content).toContain(`args = ['${serverPath}']`)
    expect(content).toContain(`COMPANY_DB_PATH = '${dbPath}'`)
    expect(content).toContain('COMPANY_SOURCE = "codex"')
    // Preserves existing config
    expect(content).toContain('model = "gpt-5.3-codex"')
    expect(content).toContain('personality = "pragmatic"')
  })

  it('replaces existing company section with new values', () => {
    const configPath = join(tmpDir, 'config.toml')
    writeFileSync(configPath, [
      'model = "gpt-5.3-codex"',
      '',
    ...
    expect(content).toContain("COMPANY_DB_PATH = '/new/data.db'")
    // Old values removed
  ...
  it('handles Windows backslash paths via TOML literal strings', () => {
    ...
    const winNode = 'C:\\Users\\test\\AppData\\Local\\company\\runtime\\node.exe'
    const winServer = 'C:\\Users\\test\\AppData\\Local\\company\\lib\\server.js'
    const winDb = 'C:\\Users\\test\\.company\\data.db'
    ...
    expect(content).toContain(`COMPANY_DB_PATH = '${winDb}'`)
  })
})
...
describe('patchClaudeCodePermissions', () => {
  ...
  it('adds mcp__company__* to existing permissions.allow', () => {
    ...
    expect(settings.permissions.allow).toContain('mcp__company__*')
  ...
  it('returns false (no-op) when already present', () => {
    ...
    mkdirSync(claudeDir, { recursive: true })
    writeFileSync(join(claudeDir, 'settings.json'), JSON.stringify({
      permissions: { allow: ['Bash(*)', 'mcp__company__*'] }
    }))
  ...
  it('creates permissions.allow array if missing', () => {
    ...
    const settings = JSON.parse(readFileSync(join(claudeDir, 'settings.json'), 'utf-8'))
    expect(settings.permissions.allow).toEqual(['mcp__company__*'])
  })
...
describe('startServer — MCP registration', () => {
  it('writes company to claude.json when file exists', async () => {
    ...
    // Also need COMPANY_DB_PATH to be set (otherwise server falls back to homedir default)
    const fakeDbPath = join(tmpDir, 'data.db')
    const origDbPath = process.env.COMPANY_DB_PATH
    process.env.COMPANY_DB_PATH = fakeDbPath
    delete process.env.COMPANY_SKIP_MCP_REGISTER
    ...
    try {
      ...
      patchMcpConfig(claudeJsonPath, {
        ...
        env: { COMPANY_DB_PATH: fakeDbPath, COMPANY_SOURCE: 'claude-code' }
      })

      const config = JSON.parse(readFileSync(claudeJsonPath, 'utf-8'))
      expect(config.mcpServers.company).toBeDefined()
      expect(config.mcpServers.company.env.COMPANY_DB_PATH).toBe(fakeDbPath)
      expect(config.mcpServers.company.env.COMPANY_SOURCE).toBe('claude-code')
      expect(config.mcpServers.daymon).toBeDefined() // other servers preserved
    } finally {
      process.env.HOME = origHome
      if (origDbPath !== undefined) {
        process.env.COMPANY_DB_PATH = origDbPath
      } else {
        delete process.env.COMPANY_DB_PATH
      }
    }
  })
  ...
  it('patches all four clients when their configs exist', () => {
    ...
    const entry = (source: string) => ({ command: 'node', args: ['s.js'], env: { COMPANY_SOURCE: source } })
    ...
    for (const [client, p] of Object.entries(configs)) {
      ...
      expect(config.mcpServers.company, client).toBeDefined()
    }

    // Verify each got its own COMPANY_SOURCE tag
    expect(JSON.parse(readFileSync(configs.claudeCode, 'utf-8')).mcpServers.company.env.COMPANY_SOURCE).toBe('claude-code')
    expect(JSON.parse(readFileSync(configs.cursor, 'utf-8')).mcpServers.company.env.COMPANY_SOURCE).toBe('cursor')
  })
})
