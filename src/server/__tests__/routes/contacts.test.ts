  ...
  it('verifies telegram via cloud bridge token status', async () => {
    ...
    vi.spyOn(globalThis, 'fetch').mockImplementation(async (input, init) => {
      ...
      if (url.endsWith('/telegram/verify/start')) {
        ...
        return jsonResponse({ ok: true, botUsername: 'company_ai_bot' })
      }
      if (url.endsWith(`/telegram/verify/status/${encodeURIComponent(pendingTokenHash)}`)) {
        return jsonResponse({
          ok: true,
          status: 'verified',
          botUsername: 'company_ai_bot',
          telegram: {
            id: '123456',
            username: 'keeper_tg',
            firstName: 'Keeper',
    ...
    expect(start.status).toBe(200)
    expect((start.body as { deepLink?: string }).deepLink).toContain('https://t.me/company_ai_bot?start=tv1_')
