...
describe('Server integration', () => {
  ...
  describe('Cloud update behavior', () => {
    it('blocks installer download route in cloud deployment', async () => {
      const prevDeployment = process.env.COMPANY_DEPLOYMENT_MODE
      ...
      try {
        ...
        cloudCtx?.close()
        if (prevDeployment === undefined) delete process.env.COMPANY_DEPLOYMENT_MODE
        else process.env.COMPANY_DEPLOYMENT_MODE = prevDeployment
      }
    })
  })
  ...
  describe('Clerk contact ingest', () => {
    it('routes clerk inbox messages into unified clerk log and skips room chat', async () => {
      const prevDataDir = process.env.COMPANY_DATA_DIR
      const dataDir = mkdtempSync(join(tmpdir(), 'company-clerk-contact-'))
      process.env.COMPANY_DATA_DIR = dataDir
      ...
      try {
        ...
        vi.unstubAllGlobals()
        vi.restoreAllMocks()
        if (prevDataDir == null) delete process.env.COMPANY_DATA_DIR
        else process.env.COMPANY_DATA_DIR = prevDataDir
    ...
    it('sends telegram typing while clerk prepares telegram reply', async () => {
      const prevDataDir = process.env.COMPANY_DATA_DIR
      const dataDir = mkdtempSync(join(tmpdir(), 'company-clerk-typing-'))
      process.env.COMPANY_DATA_DIR = dataDir
