...
async function importChecker(options?: {
  ...
  const assets = options?.assets ?? makeRelease([{
    name: 'company-update-v1.2.3.tar.gz',
    browser_download_url: 'https://example.com/company-update-v1.2.3.tar.gz',
  }])[0].assets
...
describe('updateChecker', () => {
  ...
  it('does not fire onReadyUpdate when release has no update bundle', async () => {
    ...
    const { mod, checkAndApplyUpdate } = await importChecker({
      assets: [{
        name: 'company-v1.2.3-darwin-universal.pkg',
        browser_download_url: 'https://example.com/company-v1.2.3.pkg',
      }],
      readyVersion: null,
    })
