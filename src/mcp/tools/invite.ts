...
export function registerInviteTools(server: McpServer): void {
  ...
  server.registerTool(
    ...
    async ({ roomId }) => {
      ...
      if (invites.length === 0) {
        return { content: [{ type: 'text' as const, text: 'No invite links found. Create one with company_invite_create.' }] }
      }
    ...
    async ({ roomId }) => {
      ...
      if (rooms.length === 0) {
        return {
          content: [{
            type: 'text' as const,
            text: 'No rooms in your network yet. Create an invite link with company_invite_create and share it to grow your network.'
          }]
        }
      }
      const summary = [
