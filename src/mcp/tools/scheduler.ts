...
function getServerPort(): number | null {
  try {
    const dbPath = process.env.COMPANY_DB_PATH
    const dataDir = process.env.COMPANY_DATA_DIR || (dbPath ? dirname(dbPath) : join(homedir(), `.${APP_NAME.toLowerCase()}`))
...
export function registerSchedulerTools(server: McpServer): void {
  server.registerTool(
    ...
    async ({ name, prompt, cronExpression, scheduledAt, description, maxRuns, workerId, sessionContinuity, timeout, maxTurns, allowedTools, disallowedTools, triggerType: triggerTypeInput, roomId }) => {
      ...
      if (workerId) {
        ...
        if (!worker) {
          return {
            content: [{
              type: 'text' as const,
              text: `No worker found with id ${workerId}. Use company_list_workers to see valid IDs.`
            }],
            isError: true
          }
        }
      ...
      const task = queries.createTask(db, {
        ...
        triggerType,
        triggerConfig: JSON.stringify({ source: process.env.COMPANY_SOURCE || 'claude-desktop' }),
        webhookToken,
        description,
  ...
  server.registerTool(
    ...
    {
      title: 'Run Task Now',
      description: 'Execute a task immediately. Returns right away — use company_task_progress to check status.',
      inputSchema: {
    ...
    async ({ id }) => {
      ...
      if (isTaskRunning(id)) {
        return {
          content: [{ type: 'text' as const, text: `Task "${task.name}" is already running. Use company_task_progress to check status.` }]
        }
      }
      ...
      const resultsDir = process.env.COMPANY_RESULTS_DIR || join(homedir(), APP_NAME, 'results')
      ...
      executeTask(id, { db, resultsDir })
        .then((result) => {
          ...
          try {
            const dbPath = process.env.COMPANY_DB_PATH
            if (dbPath) {
              const dataDir = process.env.COMPANY_DATA_DIR || dirname(dbPath)
      ...
      return {
        content: [{
          type: 'text' as const,
          text: `Task "${task.name}" started. Use company_task_progress to check status.`
        }]
      }
    }
  )
