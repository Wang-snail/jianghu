...
export function registerRoomTools(server: McpServer): void {
  server.registerTool(
    ...
    async ({ name, goal, queenSystemPrompt, threshold, timeoutMinutes }) => {
      ...
      const result = await createRoom(db, { name: normalizedName, goal, queenSystemPrompt, config: config as Parameters<typeof createRoom>[1]['config'] })
