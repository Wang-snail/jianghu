...
describe('tool registration', () => {
  it('registers all 3 identity tools', () => {
    expect(toolHandlers.has('company_identity_register')).toBe(true)
    expect(toolHandlers.has('company_identity_get')).toBe(true)
    expect(toolHandlers.has('company_identity_update')).toBe(true)
  })
})

describe('company_identity_register', () => {
  it('registers identity for a room with wallet', async () => {
    createRoomWallet(db, roomId, 'test-key')
    const handler = toolHandlers.get('company_identity_register')!
...
describe('company_identity_get', () => {
  it('returns null-ish for room without identity', async () => {
    createRoomWallet(db, roomId, 'test-key')
    const handler = toolHandlers.get('company_identity_get')!
...
describe('company_identity_update', () => {
  it('updates identity URI', async () => {
    createRoomWallet(db, roomId, 'test-key')
    const handler = toolHandlers.get('company_identity_update')!
    const result = await handler({ roomId, encryptionKey: 'test-key' })
    expect(result.isError).toBeUndefined()
