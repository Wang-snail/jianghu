...
import type { RoomActivityEntry } from '../../../shared/types'
...
//   6. NOT contain "company_run_task" or other tool names
...
describe('company_schedule responses', () => {
  it('cron task: clean, no implementation details', async () => {
    const handler = toolHandlers.get('company_schedule')!
  ...
  it('manual task: clean, no implementation details', async () => {
    ...
    expect(text).not.toContain('company_run_task')
  })
...
describe('company_pause_task response', () => {
  it('clean confirmation, no ID', async () => {
    ...
    const handler = toolHandlers.get('company_pause_task')!
...
describe('company_resume_task response', () => {
  it('clean confirmation, no ID', async () => {
    const task = queries.createTask(db, {
      name: 'My Task',
      prompt: 'do stuff',
    ...
    const handler = toolHandlers.get('company_resume_task')!
    const result = await handler({ id: task.id })
    const text = getResponseText(result)
    assertCleanResponse(text)
    expect(text).toContain('My Task')
...
describe('company_delete_task response', () => {
  it('clean confirmation, no ID', async () => {
    const task = queries.createTask(db, {
      name: 'My Task',
      prompt: 'do stuff',
    ...
    const handler = toolHandlers.get('company_delete_task')!
    const result = await handler({ id: task.id })
    const text = getResponseText(result)
    assertCleanResponse(text)
    expect(text).toContain('My Task')
...
describe('company_reset_session response', () => {
  it('clean confirmation, no ID', async () => {
    const task = queries.createTask(db, {
      name: 'Session Task',
      prompt: 'do stuff',
    ...
    const handler = toolHandlers.get('company_reset_session')!
    const result = await handler({ id: task.id })
    const text = getResponseText(result)
    assertCleanResponse(text)
...
describe('company_remember response', () => {
  it('clean confirmation, no ID or category', async () => {
    const handler = toolHandlers.get('company_remember')!
    const result = await handler({
      name: 'favorite color',
...
describe('company_forget response', () => {
  it('clean confirmation, no ID', async () => {
    const entity = queries.createEntity(db, 'old memory', 'fact', 'work')
    const handler = toolHandlers.get('company_forget')!
...
describe('company_create_worker response', () => {
  it('clean confirmation, no ID or default note', async () => {
    const handler = toolHandlers.get('company_create_worker')!
    const result = await handler({
      name: 'News Curator',
...
describe('company_update_worker response', () => {
  it('clean confirmation, no ID', async () => {
    const worker = queries.createWorker(db, {
      name: 'Old Name',
      systemPrompt: 'prompt'
    })
    const handler = toolHandlers.get('company_update_worker')!
...
describe('company_list_workers response', () => {
  it('includes role for each worker', async () => {
    ...
    const handler = toolHandlers.get('company_list_workers')!
    const result = await handler({})
    const text = getResponseText(result)
...
describe('company_delete_worker response', () => {
  it('clean confirmation, no ID', async () => {
    const worker = queries.createWorker(db, {
      name: 'Doomed Worker',
      systemPrompt: 'prompt'
    })
    const handler = toolHandlers.get('company_delete_worker')!
...
describe('company_watch response', () => {
  it('clean confirmation, no ID', async () => {
    const testPath = join(homedir(), 'test-downloads')
    const handler = toolHandlers.get('company_watch')!
    const result = await handler({
      path: testPath,
...
describe('company_unwatch response', () => {
  it('clean confirmation, no ID', async () => {
    const watch = queries.createWatch(db, '/Users/test/docs', 'test watch', 'do stuff')
    const handler = toolHandlers.get('company_unwatch')!
...
describe('company_create_room', () => {
  it('creates room with goal', async () => {
    const handler = toolHandlers.get('company_create_room')!
...
describe('company_list_rooms', () => {
  it('returns empty when no rooms', async () => {
    const handler = toolHandlers.get('company_list_rooms')!
    const result = await handler({})
    expect(getResponseText(result)).toContain('No rooms')
  })

  it('lists rooms after creation', async () => {
    await toolHandlers.get('company_create_room')!({ name: 'rooma' })
    const handler = toolHandlers.get('company_list_rooms')!
    const result = await handler({})
    const text = getResponseText(result)
    const data = JSON.parse(text)
  ...
  it('filters by status', async () => {
    await toolHandlers.get('company_create_room')!({ name: 'activeroom' })
    const handler = toolHandlers.get('company_list_rooms')!
...
describe('company_pause_room', () => {
  it('pauses an active room', async () => {
    await toolHandlers.get('company_create_room')!({ name: 'room' })
    const rooms = queries.listRooms(db)
    const handler = toolHandlers.get('company_pause_room')!
...
describe('company_restart_room', () => {
  it('restarts with new goal', async () => {
    await toolHandlers.get('company_create_room')!({ name: 'room', goal: 'Old goal' })
    const rooms = queries.listRooms(db)
    const handler = toolHandlers.get('company_restart_room')!
    const result = await handler({ id: rooms[0].id, newGoal: 'New goal' })
    expect(getResponseText(result)).toContain('New goal')
  })
})

describe('company_delete_room', () => {
  it('deletes a room', async () => {
    await toolHandlers.get('company_create_room')!({ name: 'Doomed' })
    const rooms = queries.listRooms(db)
    const handler = toolHandlers.get('company_delete_room')!
    const result = await handler({ id: rooms[0].id })
...
describe('company_room_status', () => {
  it('returns room status with workers and goals', async () => {
    await toolHandlers.get('company_create_room')!({ name: 'testroom', goal: 'Make money' })
    const rooms = queries.listRooms(db)
    const handler = toolHandlers.get('company_room_status')!
    const result = await handler({ id: rooms[0].id })
...
describe('company_room_activity', () => {
  it('returns activity feed', async () => {
    await toolHandlers.get('company_create_room')!({ name: 'activeroom' })
    const rooms = queries.listRooms(db)
    const handler = toolHandlers.get('company_room_activity')!
    const result = await handler({ id: rooms[0].id })
    const data = JSON.parse(getResponseText(result))
    expect(data.length).toBeGreaterThan(0)
    // First entry is most recent (wallet creation = financial), room creation = system
    expect(data.some((d: RoomActivityEntry) => d.eventType === 'system')).toBe(true)
  })
...
describe('company_propose', () => {
  let roomId: number
  let queenId: number

  beforeEach(async () => {
    await toolHandlers.get('company_create_room')!({ name: 'voteroom', goal: 'test' })
    const rooms = queries.listRooms(db)
    roomId = rooms[0].id
  ...
  it('creates a voting proposal', async () => {
    const handler = toolHandlers.get('company_propose')!
    const result = await handler({
...
describe('company_vote', () => {
  ...
  it('casts a vote and resolves decision', async () => {
    await toolHandlers.get('company_create_room')!({ name: 'VR', goal: 'test' })
    const rooms = queries.listRooms(db)
    const roomId = rooms[0].id
    const queenId = rooms[0].queenWorkerId!
  ...
  it('keeps announced decisions effective when no objection is raised before expiry sweep', async () => {
    await toolHandlers.get('company_create_room')!({ name: 'EffRoom', goal: 'test' })
    const rooms = queries.listRooms(db)
    const roomId = rooms[0].id
    const queenId = rooms[0].queenWorkerId!
...
describe('company_list_decisions', () => {
  it('returns no decisions message', async () => {
    await toolHandlers.get('company_create_room')!({ name: 'DR' })
    const rooms = queries.listRooms(db)
...
describe('company_decision_detail', () => {
  it('returns decision with votes', async () => {
    await toolHandlers.get('company_create_room')!({ name: 'DD' })
    const rooms = queries.listRooms(db)
    const decision = queries.createDecision(db, rooms[0].id, rooms[0].queenWorkerId!, 'Test?', 'strategy')

    const handler = toolHandlers.get('company_decision_detail')!
...
describe('company_set_goal', () => {
  it('sets room objective', async () => {
    await toolHandlers.get('company_create_room')!({ name: 'GR' })
    const rooms = queries.listRooms(db)
    const handler = toolHandlers.get('company_set_goal')!
...
describe('company_create_subgoal', () => {
  it('decomposes goal into sub-goals', async () => {
    await toolHandlers.get('company_create_room')!({ name: 'SG', goal: 'Main' })
    const rooms = queries.listRooms(db)
    const goals = queries.listGoals(db, rooms[0].id)
    const rootGoal = goals[0]

    const handler = toolHandlers.get('company_create_subgoal')!
...
describe('company_update_progress', () => {
  it('logs progress observation', async () => {
    await toolHandlers.get('company_create_room')!({ name: 'PR', goal: 'Build' })
    const rooms = queries.listRooms(db)
    const goals = queries.listGoals(db, rooms[0].id)

    const handler = toolHandlers.get('company_update_progress')!
...
describe('company_complete_goal', () => {
  it('marks goal as completed', async () => {
    await toolHandlers.get('company_create_room')!({ name: 'CG', goal: 'Do' })
    const rooms = queries.listRooms(db)
    const goals = queries.listGoals(db, rooms[0].id)

    const handler = toolHandlers.get('company_complete_goal')!
    const result = await handler({ goalId: goals[0].id })
    expect(getResponseText(result)).toContain('completed')
  })
})

describe('company_abandon_goal', () => {
  it('marks goal as abandoned with reason', async () => {
    await toolHandlers.get('company_create_room')!({ name: 'AG', goal: 'Bad idea' })
    const rooms = queries.listRooms(db)
    const goals = queries.listGoals(db, rooms[0].id)

    const handler = toolHandlers.get('company_abandon_goal')!
    const result = await handler({ goalId: goals[0].id, reason: 'Market changed' })
    expect(getResponseText(result)).toContain('Market changed')
  })
})

describe('company_list_goals', () => {
  it('returns tree format', async () => {
    await toolHandlers.get('company_create_room')!({ name: 'LG', goal: 'Root goal' })
    const rooms = queries.listRooms(db)
    const handler = toolHandlers.get('company_list_goals')!
    const result = await handler({ roomId: rooms[0].id })
...
describe('company_create_skill', () => {
  let roomId: number

  beforeEach(async () => {
    await toolHandlers.get('company_create_room')!({ name: 'SR' })
    roomId = queries.listRooms(db)[0].id
  })

  it('creates a skill without worker', async () => {
    const handler = toolHandlers.get('company_create_skill')!
    const result = await handler({
...
describe('company_list_skills', () => {
  it('returns no skills message', async () => {
    await toolHandlers.get('company_create_room')!({ name: 'LS' })
    const roomId = queries.listRooms(db)[0].id
    const handler = toolHandlers.get('company_list_skills')!
    const result = await handler({ roomId })
    expect(getResponseText(result)).toContain('No skills')
  })
})

describe('company_activate_skill', () => {
  it('activates a deactivated skill', async () => {
    await toolHandlers.get('company_create_room')!({ name: 'AS' })
    const roomId = queries.listRooms(db)[0].id
    const skill = queries.createSkill(db, roomId, 'Sk', 'c', { autoActivate: false })

    const handler = toolHandlers.get('company_activate_skill')!
...
describe('company_deactivate_skill', () => {
  it('deactivates a skill', async () => {
    await toolHandlers.get('company_create_room')!({ name: 'DS' })
    const roomId = queries.listRooms(db)[0].id
    const skill = queries.createSkill(db, roomId, 'Sk', 'c', { autoActivate: true })

    const handler = toolHandlers.get('company_deactivate_skill')!
    const result = await handler({ skillId: skill.id })
...
describe('company_delete_skill', () => {
  it('deletes a skill', async () => {
    await toolHandlers.get('company_create_room')!({ name: 'DLS' })
    const roomId = queries.listRooms(db)[0].id
    const skill = queries.createSkill(db, roomId, 'Dead Skill', 'c', {})

    const handler = toolHandlers.get('company_delete_skill')!
    const result = await handler({ skillId: skill.id })
...
describe('company_self_mod_edit', () => {
  let roomId: number
  let queenId: number
  ...
  beforeEach(async () => {
    ...
    _resetRateLimit()
    await toolHandlers.get('company_create_room')!({ name: 'SM' })
    const rooms = queries.listRooms(db)
    roomId = rooms[0].id
    queenId = rooms[0].queenWorkerId!
  })
  ...
  it('logs a general modification', async () => {
    const handler = toolHandlers.get('company_self_mod_edit')!
    const result = await handler({
