...
describe('company_wallet_create', () => {
  it('creates a wallet for a room', async () => {
    const handler = toolHandlers.get('company_wallet_create')!
...
describe('company_wallet_address', () => {
  it('returns wallet address', async () => {
    const createHandler = toolHandlers.get('company_wallet_create')!
    await createHandler({ roomId, encryptionKey: 'test-key' })

    const handler = toolHandlers.get('company_wallet_address')!
...
describe('company_wallet_history', () => {
  it('returns empty for no transactions', async () => {
    const createHandler = toolHandlers.get('company_wallet_create')!
    await createHandler({ roomId, encryptionKey: 'test-key' })

    const handler = toolHandlers.get('company_wallet_history')!
    const result = await handler({ roomId })
...
describe('tool registration', () => {
  it('registers all 5 wallet tools', () => {
    expect(toolHandlers.has('company_wallet_create')).toBe(true)
    expect(toolHandlers.has('company_wallet_address')).toBe(true)
    expect(toolHandlers.has('company_wallet_balance')).toBe(true)
    expect(toolHandlers.has('company_wallet_send')).toBe(true)
    expect(toolHandlers.has('company_wallet_history')).toBe(true)
  })
})
