...
beforeEach(async () => {
  toolHandlers.clear()
  db = initTestDb()
  process.env.COMPANY_DEPLOYMENT_MODE = 'cloud'
...
afterEach(() => {
  db.close()
  vi.clearAllMocks()
  delete process.env.COMPANY_DEPLOYMENT_MODE
})

describe('company_station_create', () => {
  it('returns checkout URL', async () => {
    const handler = toolHandlers.get('company_station_create')!
    const result = await handler({ roomId, name: 'web-server', tier: 'small' })
    expect(result.isError).toBeUndefined()
    const text = getResponseText(result)
    expect(text).toContain('http://127.0.0.1:4700/stations?room=')
    expect(text).toContain('mock-cloud-id')
  })
})

describe('company_station_list', () => {
  it('returns stations from cloud', async () => {
    const handler = toolHandlers.get('company_station_list')!
  ...
  it('returns empty message when no stations', async () => {
    ...
    const handler = toolHandlers.get('company_station_list')!
    const result = await handler({ roomId })
    expect(getResponseText(result)).toBe('未找到执行站。')
  })
})

describe('company_station_start', () => {
  it('requests station start', async () => {
    const handler = toolHandlers.get('company_station_start')!
    const result = await handler({ roomId, id: 1 })
    expect(result.isError).toBeUndefined()
    expect(getResponseText(result)).toContain('已请求启动')
  })
})

describe('company_station_stop', () => {
  it('requests station stop', async () => {
    const handler = toolHandlers.get('company_station_stop')!
    const result = await handler({ roomId, id: 1 })
    expect(result.isError).toBeUndefined()
    expect(getResponseText(result)).toContain('已请求停止')
  })
})

describe('company_station_delete', () => {
  it('requests station deletion', async () => {
    const handler = toolHandlers.get('company_station_delete')!
    const result = await handler({ roomId, id: 1 })
    expect(result.isError).toBeUndefined()
    expect(getResponseText(result)).toContain('已请求删除')
  })
})

describe('company_station_cancel', () => {
  it('requests station cancellation at end of billing period', async () => {
    const handler = toolHandlers.get('company_station_cancel')!
    const result = await handler({ roomId, id: 1 })
    expect(result.isError).toBeUndefined()
    const text = getResponseText(result)
    expect(text).toContain('已请求取消')
  })
...
describe('company_station_exec', () => {
  it('executes command and returns result', async () => {
    const handler = toolHandlers.get('company_station_exec')!
...
describe('company_station_logs', () => {
  it('returns station logs', async () => {
    const handler = toolHandlers.get('company_station_logs')!
    const result = await handler({ roomId, id: 1 })
    expect(result.isError).toBeUndefined()
...
describe('company_station_status', () => {
  it('returns station status', async () => {
    const handler = toolHandlers.get('company_station_status')!
    const result = await handler({ roomId, id: 1 })
    expect(result.isError).toBeUndefined()
    const parsed = JSON.parse(getResponseText(result))
...
describe('tool registration', () => {
  it('registers all 9 station tools', () => {
    expect(toolHandlers.has('company_station_create')).toBe(true)
    expect(toolHandlers.has('company_station_list')).toBe(true)
    expect(toolHandlers.has('company_station_start')).toBe(true)
    expect(toolHandlers.has('company_station_stop')).toBe(true)
    expect(toolHandlers.has('company_station_delete')).toBe(true)
    expect(toolHandlers.has('company_station_cancel')).toBe(true)
    expect(toolHandlers.has('company_station_exec')).toBe(true)
    expect(toolHandlers.has('company_station_logs')).toBe(true)
    expect(toolHandlers.has('company_station_status')).toBe(true)
    expect(toolHandlers.has('company_station_deploy')).toBe(false)
    expect(toolHandlers.has('company_station_domain')).toBe(false)
  })
})
