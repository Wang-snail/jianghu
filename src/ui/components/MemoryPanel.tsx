...
export function MemoryPanel({ roomId }: MemoryPanelProps): React.JSX.Element {
  ...
  function renderObservations(entityId: number): React.JSX.Element | null {
    ...
    if (!obs) return <div className="text-sm text-text-muted py-1">加载中...</div>
    if (obs.length === 0) return <div className="text-sm text-text-muted py-1">暂无观察记录</div>
    return (
