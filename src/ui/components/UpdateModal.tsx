...
export function UpdateModal({
  ...
  return (
    <div
      ...
      onClick={(e) => { if (e.target === e.currentTarget && !restarting) onDismiss() }}
    >
      <div className="bg-surface-primary rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-6 relative">
        ...
        <div className="pr-4">
          {restarting ? (
            <>
              <p className="text-xs font-semibold text-status-info uppercase tracking-wide mb-1">Updating</p>
              <h2 className="text-xl font-bold text-text-primary mb-1">Restarting...</h2>
              <p className="text-sm text-text-muted mb-5">Applying v{version}. The page will reload automatically.</p>
            ...
            <>
              <p className="text-xs font-semibold text-status-success uppercase tracking-wide mb-1">
                {updateReady ? 'Update Ready' : 'Update Available'}
              </p>
              <h2 className="text-xl font-bold text-text-primary mb-1">江湖 v{version}</h2>
