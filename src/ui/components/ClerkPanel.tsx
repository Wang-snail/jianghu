...
export function ClerkPanel({ setupLaunchKey = 0 }: ClerkPanelProps): React.JSX.Element {
  ...
  const [commentaryPace, setCommentaryPace] = useState<'活跃' | 'light'>('light')
  ...
  // Keep status in sync, but do not interrupt startup with an automatic setup popup.
  // Users can still open the setup guide from the visible buttons in this panel.
  useEffect(() => {
    if (clerkStatus) {
  ...
  async function handleToggleCommentaryMode(): Promise<void> {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : '更新播报模式失败')
    } finally {
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex items-center px-4 py-3 border-b border-border-primary bg-surface-primary shrink-0">
        <div className="flex items-center gap-2">
          <span className="text-[17px] font-semibold text-text-primary">秘书</span>
          {clerkModel && (
          ...
          <span className={`inline-flex items-center px-2 py-0.5 rounded text-[12px] border ${
            ...
            播报：{!commentaryEnabled ? '关闭' : commentaryMode === 'light' ? '轻量' : commentaryPace}
          </span>
          <button
            onClick={handleToggleCommentary}
            title={commentaryEnabled ? '关闭播报' : '开启播报'}
          ...
          <button
            ...
            title={commentaryMode === 'auto' ? '切换到轻量播报' : '切回自动播报'}
      ...
      >
        ...
        {messages.length === 0 && !loading ? (
          <div className="text-text-muted text-[16px] py-8 text-center">
            {clerkModel && isConnected
              ? '秘书已就绪。可以让她创建公司、分派任务、传递消息或查看运行状态。'
              : clerkModel && connectionStatus.connected === null
                ? '正在检查连接...'
                : (
                  <>
                    <div className="mb-3">{clerkModel ? `模型（${clerkModel}）尚未连接。` : '连接模型后即可使用秘书。'}</div>
                    <button
                      onClick={() => setShowSetup(true)}
        ...
        {loading && (
          <div className="text-[16px] text-text-muted animate-pulse mb-4">处理中...</div>
        )}
        {error && (
      ...
      <div className="flex items-center gap-2 w-full max-w-[920px]">
        <input
          ...
          placeholder={!clerkModel ? '请先连接模型...' : !isConnected ? '模型未连接，请打开设置...' : '让秘书做事，例如创建公司、分派任务、查看阻塞...'}
