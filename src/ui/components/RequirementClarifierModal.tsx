...
const FIRST_AGENT_MESSAGE: ClarifierMessage = {
  id: 1,
  speaker: 'agent',
  content: `我是需求澄清师。我的职责不是直接开工，而是先把你的委托问清楚，整理成需求文档。${CLARIFIER_STEPS[0].question}`,
}
...
export function RequirementClarifierModal({ onClose, onCreate }: RequirementClarifierModalProps): React.JSX.Element {
  ...
  const [messages, setMessages] = useState<ClarifierMessage[]>([FIRST_AGENT_MESSAGE])
      ...
      >
        <div className="flex items-center justify-between gap-3 border-b border-border-primary px-4 py-3">
          <div>
            ...
            <p className="mt-0.5 text-xs text-text-muted">先由需求澄清师对话确认，再生成需求文档，最后交给天机阁调度。</p>
          </div>
          <button
            onClick={onClose}
          ...
          <div className="flex min-h-0 flex-col border-b border-border-primary lg:border-b-0 lg:border-r">
            <div className="border-b border-border-primary px-4 py-3">
              <div className="flex items-center justify-between gap-2">
                <div>
                  <div className="text-sm font-semibold text-text-primary">需求澄清师</div>
                  <div className="text-xs text-text-muted">专门负责问清楚目标、资料、交付物和验收标准。</div>
                </div>
            ...
            <div className="border-t border-border-primary bg-surface-secondary p-3">
              <textarea
                ...
                placeholder={stage === 'review' ? '需求文档已生成。需要补充时点“继续澄清”。' : '写下你的回答，按 Cmd/Ctrl + Enter 发送。'}
              ...
              <div className="mt-2 flex flex-wrap items-center gap-2">
                ...
                <span className="text-xs text-text-muted">需求没有确认前，不会交给天机阁开工。</span>
              </div>
            </div>
          </div>
