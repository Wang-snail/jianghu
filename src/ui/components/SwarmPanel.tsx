...
import type { Room, Worker, Station, RevenueSummary, OnChainBalance, Task } from '@shared/types'
import { RequirementClarifierModal } from './RequirementClarifierModal'
...
interface SwarmPanelProps {
  ...
  onRoomCreated: (room: Room) => void | Promise<void>
...
function fmtMoney(n: number): string {
  if (n === 0) return '0 财气'
  if (Math.abs(n) >= 1000) return `${(n / 1000).toFixed(1)}k 财气`
  if (Math.abs(n) >= 1) return `${n.toFixed(0)} 财气`
...
type OverviewStatus = 'running' | 'preparing' | 'completed'
type WorkerChipState = 'done' | 'running' | 'waiting' | 'blocked'

function isBusyWorker(worker: Worker): boolean {
  return worker.agentState === 'thinking' || worker.agentState === 'acting' || worker.agentState === 'voting'
}

function roomOverviewStatus(room: Room, running: boolean, tasks: Task[], workers: Worker[]): OverviewStatus {
  if (room.status === 'stopped') return 'completed'
  if (tasks.length > 0 && tasks.every(task => task.status === 'completed')) return 'completed'
  if (room.status === 'active' && (running || tasks.some(task => task.status === 'active') || workers.some(isBusyWorker))) {
    return 'running'
  }
  return 'preparing'
}

function overviewStatusMeta(status: OverviewStatus): {
  label: string
  dotClass: string
  borderClass: string
  cardClass: string
  ...
  if (status === 'running') {
    return {
      label: '执行中',
      dotClass: 'bg-[#24b88f]',
      borderClass: 'border-l-[#8b78ff]',
      cardClass: 'border-border-primary bg-surface-secondary',
      pillClass: 'bg-[#e7e4ff] text-[#4f3ecf]',
      barClass: 'bg-[#8b78ff]',
    }
  }
  if (status === 'preparing') {
    return {
      ...
      cardClass: 'border-border-primary bg-surface-secondary',
      pillClass: 'bg-[#fff0d4] text-[#a56700]',
  ...
  return {
    ...
    cardClass: 'border-border-primary bg-surface-secondary opacity-75',
    pillClass: 'bg-surface-tertiary text-text-muted',
    barClass: 'bg-text-muted',
  }
}

function relativeTimeLabel(value: string | null | undefined, suffix = '前'): string {
  if (!value) return '刚刚'
  ...
  const day = 24 * hour
  if (diffMs < minute) return suffix === '前' ? '刚刚' : '不到1分钟'
  if (diffMs < hour) return `${Math.floor(diffMs / minute)}分钟${suffix}`
  if (diffMs < day) return `${Math.floor(diffMs / hour)}小时${suffix}`
  return `${Math.floor(diffMs / day)}天${suffix}`
}

function progressPercent(status: OverviewStatus, tasks: Task[], workers: Worker[]): number {
  if (status === 'completed') return 100
  if (status === 'preparing') {
    const base = workers.length > 0 ? 18 : 8
    const taskBoost = tasks.length > 0 ? 12 : 0
    return Math.min(38, base + taskBoost)
  }
  if (tasks.length === 0) return workers.some(isBusyWorker) ? 35 : 18
  const total = tasks.length
  const completed = tasks.filter(task => task.status === 'completed').length
  const activeCredit = tasks.filter(task => task.status === 'active').reduce((sum, task) => {
    if (task.maxRuns && task.maxRuns > 0) return sum + Math.min(0.9, task.runCount / task.maxRuns)
    return sum + Math.min(0.75, task.runCount > 0 ? 0.45 : 0.2)
  }, 0)
  return Math.min(98, Math.max(8, Math.round(((completed + activeCredit) / total) * 100)))
}

function budgetLabel(status: OverviewStatus, tasks: Task[], revenue?: RevenueSummary): string {
  if (status === 'preparing') return '预算待领取'
  if (status === 'completed') return '预算已结算'
  if (revenue && (revenue.totalIncome > 0 || revenue.totalExpenses > 0)) {
    const total = Math.max(1, revenue.totalIncome + revenue.totalExpenses)
    return `预算 ${Math.min(99, Math.round((revenue.totalExpenses / total) * 100))}%`
  }
  const runs = tasks.reduce((sum, task) => sum + task.runCount, 0)
  return `预算 ${Math.min(92, Math.max(18, 18 + runs * 6))}%`
}

function estimateLabel(status: OverviewStatus, tasks: Task[], room: Room): string {
  if (status === 'completed') return `用时 ${Math.max(1, Math.round((new Date(room.updatedAt).getTime() - new Date(room.createdAt).getTime()) / 60000))} 分钟`
  if (status === 'preparing') return '预计 3 分钟后启动'
  const activeCount = tasks.filter(task => task.status === 'active').length
  return `预计还需 ${Math.max(6, activeCount * 7 + 5)} 分钟`
}

function currentActionText(status: OverviewStatus, tasks: Task[], workers: Worker[]): string {
  const activeTasks = tasks.filter(task => task.status === 'active')
  const completedTasks = tasks.filter(task => task.status === 'completed')
  if (status === 'preparing') {
    return '当前：天机处正在任命帮主，客栈挑选弟子，藏经阁领取技能包中'
  }
  if (status === 'completed') {
    const last = completedTasks[completedTasks.length - 1] ?? tasks[tasks.length - 1]
    return last
      ? `天机处验收通过，${last.name}结果已输出，经验已归档至藏经阁`
      : '天机处验收通过，结果已输出，经验已归档至藏经阁'
  }
  if (activeTasks.length >= 2) {
    const [first, second] = activeTasks
    return `当前：${first.name} + ${second.name} 并行推进，等待帮主整合`
  }
  const task = activeTasks[0] ?? tasks.find(item => item.status !== 'completed') ?? tasks[0]
  if (!task) {
    const busy = workers.find(isBusyWorker)
    return busy ? `当前：${busy.name}正在处理帮派委托` : '当前：帮主正在拆解委托，等待弟子接镖'
  }
  const owner = workers.find(worker => worker.id === task.workerId)
  const ownerName = owner?.name ?? '弟子'
  if (task.maxRuns && task.maxRuns > 0) {
    return `当前：${task.name}，${ownerName}已处理 ${Math.min(task.runCount, task.maxRuns)}/${task.maxRuns} 次`
  }
  return `当前：${task.name}，${ownerName}正在推进`
}

function workerChipState(worker: Worker, tasks: Task[]): WorkerChipState {
  if (worker.agentState === 'blocked' || worker.agentState === 'rate_limited') return 'blocked'
  const assigned = tasks.filter(task => task.workerId === worker.id)
  if (assigned.some(task => task.status === 'active') || isBusyWorker(worker)) return 'running'
  if (assigned.length > 0 && assigned.every(task => task.status === 'completed')) return 'done'
  return 'waiting'
}

function workerChipMeta(state: WorkerChipState): { label: string; dotClass: string; className: string } {
  if (state === 'done') return { label: '完成', dotClass: 'bg-[#24b88f]', className: 'border-status-success/40 text-text-secondary' }
  if (state === 'running') return { label: '进行中', dotClass: 'bg-[#8b78ff]', className: 'border-[#8b78ff]/50 text-text-secondary' }
  if (state === 'blocked') return { label: '受阻', dotClass: 'bg-status-warning', className: 'border-status-warning/50 text-text-secondary' }
  return { label: '待命', dotClass: 'bg-text-muted', className: 'border-border-primary text-text-muted' }
}

function completionValueText(status: OverviewStatus, tasks: Task[], workers: Worker[]): string {
  if (status !== 'completed') return ''
  const updatedWorkers = workers.length > 0 ? `${Math.min(workers.length, 4)}名弟子声望更新` : '声望已更新'
  if (tasks.some(task => task.learnedContext || task.lastResult)) {
    return `沉淀：经验归档至藏经阁，${updatedWorkers}`
  }
  return `沉淀：复盘已归档，${updatedWorkers}`
}
...
function roomColors(room: Room, running: boolean): { fill: string; stroke: string } {
  ...
  if (room.status === '已暂停') return { fill: 'var(--status-warning-bg)', stroke: 'var(--status-warning)' }
  if (room.status === '已停止') return { fill: 'var(--surface-tertiary)', stroke: 'var(--border-primary)' }
...
function stationColor(status: string): string {
  switch (status) {
    case '活跃': return 'var(--status-success-bg)'
    case '待处理': return 'var(--status-warning-bg)'
...
/** COMPANY wordmark: Q,O,O,O are hexagons; U,R,M are geometric strokes */
function addBrandFooter(svgClone: SVGSVGElement): void {
  ...
  // Product tagline to the right of the wordmark
  ...
  sub.textContent = '江湖数字组织'
  g.appendChild(sub)

  // Local-first product identity on right
...
async function shareToTwitter(svgEl: SVGSVGElement, hideMoney: boolean): Promise<void> {
  ...
  const origin = window.location.origin || 'http://127.0.0.1:4700'
  const text = encodeURIComponent(`我的江湖数字组织正在运行\n\n${origin}`)
...
const INVITE_URL = typeof window === 'undefined' ? 'http://127.0.0.1:4700' : window.location.origin

const INVITE_CARDS: InviteCard[] = [
  {
    shareId: '01',
    label: '天机阁调度',
    previewText: '用户只下委托，整个江湖自动调动帮派完成押运。',
  },
  {
    shareId: '02',
    label: '帮派协作',
    previewText: '风闻阁查情报，百工坊做交付，金算盘管财气。',
  },
  {
    shareId: '03',
    label: '数字组织生态',
    previewText: '重点不是更多模型，而是让 AI 形成有秩序的组织。',
  },
  {
    shareId: '04',
    label: '龙门镖局',
  ...
  },
  {
    shareId: '06',
    label: '功法体系',
    previewText: '每次交付都会沉淀功法，让下一次行动更稳。',
  },
  {
    shareId: '07',
    label: '藏经阁记忆',
    previewText: '经验、偏好、失败和复盘进入藏经阁，可审计可回滚。',
  },
  {
    shareId: '09',
    label: '江湖协作',
    previewText: '多方帮派，一条委托，持续押运。',
  },
  {
    shareId: 'v1',
    label: '低认知负担',
    previewText: '人负责目标和关键决策，江湖负责执行与推演。',
  },
  {
    shareId: 'v2',
    label: '钱庄资源',
...
export function SwarmPanel({ rooms, queenRunning, forcedInviteOpenNonce, onNavigateToRoom, onRoomCreated, demoData }: SwarmPanelProps): React.JSX.Element {
  ...
  const { data: tasksByRoom, refresh: refreshTasksByRoom } = usePolling<Record<number, Task[]>>(
    async () => {
      if (rooms.length === 0) return {}
      const entries = await Promise.all(
        rooms.map(async r => {
          const tasks = await api.tasks.list(r.id).catch(() => [] as Task[])
          return [r.id, tasks] as const
        })
      )
  ...
  const overviewCards = useMemo(() => {
    return rooms
      .map(room => {
        ...
        const running = queenRunning[room.id] === true
        const status = roomOverviewStatus(room, running, tasks, workers)
        const progress = progressPercent(status, tasks, workers)
        const queenWorker = workers.find(w => w.id === room.queenWorkerId)
        const chips = workers.slice(0, 6).map(worker => ({
          worker,
          state: workerChipState(worker, tasks),
        }))
        return {
          room,
          workers,
          tasks,
          status,
          meta: overviewStatusMeta(status),
          progress,
          queenWorker,
          chips,
          action: currentActionText(status, tasks, workers),
          estimate: estimateLabel(status, tasks, room),
          budget: budgetLabel(status, tasks, revenueMap?.[room.id]),
          value: completionValueText(status, tasks, workers),
        }
      })
      .sort((a, b) => {
        const rank: Record<OverviewStatus, number> = { running: 0, preparing: 1, completed: 2 }
        const diff = rank[a.status] - rank[b.status]
        if (diff !== 0) return diff
        return new Date(b.room.updatedAt).getTime() - new Date(a.room.updatedAt).getTime()
      })
  }, [queenRunning, revenueMap, rooms, tasksByRoom, workersPerRoom])

  const overviewCounts = useMemo(() => ({
    running: overviewCards.filter(card => card.status === 'running').length,
    preparing: overviewCards.filter(card => card.status === 'preparing').length,
    completed: overviewCards.filter(card => card.status === 'completed').length,
  }), [overviewCards])
  ...
  const handleCopyInviteItem = useCallback(async (value: string, label: string) => {
    ...
    setInviteCopyStatus(copied ? `${label}已复制` : `${label}复制失败`)
  ...
  const handleInviteShare = useCallback(async (platform: 'twitter' | 'instagram' | 'facebook' | 'sms' | 'telegram') => {
    ...
    switch (platform) {
      ...
      case 'telegram': {
        ...
        setShareStatus('已打开 Telegram')
        break
      }
      case 'sms': {
        const body = encodeURIComponent(shareText)
        window.open(`sms:?&body=${body}`, '_self')
        setShareStatus('已打开短信')
        break
      }
      case 'instagram': {
        const copied = await copyText(`${card.previewText}\n${shareUrl}`)
        window.open('https://www.instagram.com/', '_blank')
        setShareStatus(copied ? '文案已复制，请在 Instagram 使用。' : '已打开 Instagram。')
        break
      }
    }
  ...
  const handleShare = useCallback(async (platform: 'download' | 'twitter' | 'instagram' | 'tiktok') => {
    ...
    setShareStatus('正在生成图片...')
    const hideMoney = !showMoney
    try {
      switch (platform) {
        case 'download': await downloadImage(svgRef.current, hideMoney); setShareStatus('已下载。'); break
        case 'twitter': await shareToTwitter(svgRef.current, hideMoney); setShareStatus('图片已保存，正在打开 Twitter...'); break
        case 'instagram': await shareToInstagram(svgRef.current, hideMoney); setShareStatus('已复制到剪贴板，请粘贴到 Instagram。'); break
        case 'tiktok': await shareToTikTok(svgRef.current, hideMoney); setShareStatus('图片已保存，请上传到 TikTok。'); break
      }
    } catch { setShareStatus('图片生成失败') }
  ...
  return (
    <div ref={containerRef} className="relative flex flex-col h-full">
      {/* Header */}
      <div className="flex items-start justify-between gap-4 px-4 py-4 border-b border-border-primary shrink-0">
        <div>
          <h2 className="text-xl font-semibold text-text-primary">江湖纵览</h2>
          <div className="mt-1 text-sm text-text-muted">天机处 · 实时监控</div>
        </div>
        <div className="flex flex-wrap justify-end gap-2">
          {[
            { label: '执行中', value: overviewCounts.running, dot: 'bg-[#24b88f]' },
            { label: '筹备中', value: overviewCounts.preparing, dot: 'bg-status-warning' },
            { label: '已完成', value: overviewCounts.completed, dot: 'bg-text-muted' },
          ].map(item => (
            <div key={item.label} className="inline-flex min-w-[108px] items-center gap-2 rounded-full border border-border-primary bg-surface-tertiary px-4 py-1.5">
              <span className={`h-2.5 w-2.5 rounded-full ${item.dot}`} />
              <span className="text-sm font-medium text-text-secondary">{item.label}</span>
              <span className="ml-auto text-base font-semibold text-text-primary">{item.value}</span>
            </div>
          ))}
        </div>
      </div>
        ...
        >
          ...
          >
            <div className="flex items-center justify-between px-4 py-3 border-b border-border-primary">
              <div>
                <h3 className="text-sm font-semibold text-text-primary">江湖帖预览</h3>
                <p className="text-xs text-text-muted">分享前预览文案和图片</p>
              </div>
              <button
                onClick={() => setInviteOpen(false)}
            ...
            <div className="grid gap-4 p-4 md:grid-cols-[1.4fr_1fr]">
              <div className="flex flex-col gap-3">
                <div className="rounded-lg border border-border-primary bg-surface-secondary p-2">
                  <div className="aspect-[16/9] rounded-md border border-border-primary bg-surface-primary px-5 py-4 flex flex-col justify-between overflow-hidden">
                    <div className="flex items-center justify-between gap-3">
                      <div className="text-lg font-semibold text-text-primary">江湖</div>
                      <div className="text-[11px] tracking-[0.16em] text-text-muted uppercase">AI 数字组织</div>
                    </div>
                    <div>
                      <div className="text-[11px] tracking-[0.16em] text-interactive mb-2">{INVITE_CARDS[inviteCardIndex].label}</div>
                      <div className="text-2xl font-semibold text-text-primary leading-snug">
                        整个江湖正在替我运作
                      </div>
                      <div className="text-sm text-text-muted mt-2 leading-relaxed">
                        {INVITE_CARDS[inviteCardIndex].previewText}
                      </div>
                    </div>
                    <div className="flex items-center justify-between text-xs text-text-muted">
                      <span>天机阁 · 帮派 · 龙门镖局</span>
                      <span>{INVITE_URL.replace(/^https?:\/\//, '')}</span>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex flex-col gap-3">
                <div className="rounded-lg border border-border-primary bg-surface-secondary px-3 py-2 space-y-2">
                  <div>
                    <div className="text-[11px] uppercase tracking-[0.12em] text-text-muted mb-1">你的江湖帖码</div>
                    <div className="flex items-center gap-2">
                      <code className="flex-1 text-xs px-2 py-1 rounded bg-surface-primary border border-border-primary text-text-secondary break-all">
                        {keeperReferralCode || '暂不可用'}
                      </code>
                      <button
                        onClick={() => { if (keeperReferralCode) void handleCopyInviteItem(keeperReferralCode, '江湖帖码') }}
                  ...
                  <div>
                    <div className="text-[11px] uppercase tracking-[0.12em] text-text-muted mb-1">江湖帖链接</div>
                    <div className="flex items-center gap-2">
                      <code className="flex-1 text-xs px-2 py-1 rounded bg-surface-primary border border-border-primary text-text-secondary break-all">
                        {keeperInviteUrl}
                      </code>
                      <button
                        onClick={() => void handleCopyInviteItem(keeperInviteUrl, '链接')}
      ...
      <div className="flex-1 overflow-auto px-4 py-4">
        {overviewCards.length === 0 ? (
          <div className="flex min-h-[320px] items-center justify-center rounded-lg border border-dashed border-border-primary bg-surface-secondary text-sm text-text-muted">
            暂无帮派。创建一个临时帮派后，江湖纵览会显示它的筹备、执行和沉淀状态。
          </div>
        ) : (
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {overviewCards.map(card => {
              const roomAge = card.status === 'running'
                ? `${relativeTimeLabel(card.room.updatedAt)}启动`
                : card.status === 'preparing'
                  ? relativeTimeLabel(card.room.updatedAt)
                  : relativeTimeLabel(card.room.updatedAt)
              return (
                <button
                  key={card.room.id}
                  onClick={() => onNavigateToRoom(card.room.id)}
                  className={`group flex min-h-[270px] flex-col overflow-hidden rounded-lg border border-l-4 ${card.meta.borderClass} ${card.meta.cardClass} text-left transition-colors hover:border-interactive/60 hover:bg-surface-hover focus:outline-none focus:ring-2 focus:ring-interactive/40`}
                >
                  <div className="flex items-start justify-between gap-3 px-4 py-4">
                    <div className="min-w-0">
                      <div className="truncate text-lg font-semibold text-text-primary">{card.room.name}</div>
                      <div className="mt-3 grid grid-cols-3 gap-3 text-sm text-text-muted">
                        <div className="min-w-0">
                          <span>帮主：</span>
                          <span className="text-text-secondary">{card.queenWorker?.name ?? '待任命'}</span>
                        </div>
                        <div>
                          <span className="text-text-secondary">{card.workers.length}</span>
                          <span>名弟子</span>
                        </div>
                        <div>{roomAge}</div>
                      </div>
                    </div>
                    <span className={`inline-flex shrink-0 items-center gap-1.5 rounded-full px-3 py-1 text-sm font-medium ${card.meta.pillClass}`}>
                      <span className={`h-2.5 w-2.5 rounded-full ${card.meta.dotClass}`} />
                      {card.meta.label}
                    </span>
                  </div>
                  ...
                  <div className="border-t border-border-primary px-4 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-surface-tertiary">
                        <div className={`h-full rounded-full ${card.meta.barClass}`} style={{ width: `${card.progress}%` }} />
                      </div>
                      <span className="w-10 text-right text-sm font-medium text-text-secondary">{card.status === 'preparing' ? '--' : `${card.progress}%`}</span>
                    </div>

                    <div className="mt-3 flex flex-wrap gap-2">
                      {card.chips.length === 0 ? (
                        <span className="inline-flex items-center rounded-full border border-border-primary px-3 py-1 text-xs text-text-muted">客栈招募中...</span>
                      ) : (
                        card.chips.map(({ worker, state }) => {
                          const chip = workerChipMeta(state)
                          return (
                            <span key={worker.id} className={`inline-flex max-w-[150px] items-center gap-1.5 rounded-full border px-3 py-1 text-xs ${chip.className}`}>
                              <span className={`h-2 w-2 shrink-0 rounded-full ${chip.dotClass}`} />
                              <span className="truncate">{worker.name} {chip.label}</span>
                            </span>
                          )
                        })
                      )}
                    ...
                    <div className="mt-4 rounded-md bg-black/20 px-4 py-3 text-sm leading-6 text-text-secondary">
                      {card.action}
                    </div>
                  </div>

                  <div className="mt-auto grid grid-cols-2 gap-3 border-t border-border-primary px-4 py-3 text-sm text-text-muted">
                    <div>{card.status === 'completed' ? card.estimate : card.estimate}</div>
                    <div className="text-right">{card.status === 'completed' ? (card.value || '经验已归档') : card.budget}</div>
                  </div>
                </button>
              )
            })}
      ...
      {/* Legacy honeycomb kept for share-image internals, hidden from the product surface. */}
      {false && (
      <div className="flex-1 overflow-auto relative" onMouseMove={handleMouseMove}>
        ...
        >
          {positions.map(({ node, cx, cy }) => {
            ...
            const running = room ? (room.status === '活跃' && queenRunning[room.id]) : false
            ...
            const activeStations = stations.filter(s => s.status === '活跃').length
            ...
            return (
              ...
              >
                ...
                {/* Money line — always shown when the money toggle is on */}
                {showMoney && !isPlaceholder && (
                  <text
                    ...
                    data-money="true"
                  >
                    <tspan fill="var(--status-success)">{fmtMoney(revenue?.totalIncome ?? 0)} 入账</tspan>
                    <tspan fill="var(--text-muted)">{' / '}</tspan>
                    <tspan fill="var(--status-error)">{fmtMoney(revenue?.totalExpenses ?? 0)} 支出</tspan>
                    {(revenue?.stationCosts ?? 0) > 0 && (
                      <tspan fill="var(--text-muted)">{' \u00b7 '}</tspan>
                    )}
                    {(revenue?.stationCosts ?? 0) > 0 && (
                      <tspan fill="var(--status-warning)">{fmtMoney(revenue!.stationCosts)} 资源</tspan>
                    )}
                  </text>
                )}
          ...
          {referredPositions.map((rp) => {
            ...
            const refName = isPublic ? (rp.ref.name || rp.ref.roomId.slice(0, 12)) : '私有'
          ...
          >
            ...
            <div className="text-sm text-text-secondary mt-2 space-y-0.5">
              {(workersPerRoom[hoveredRoom.id] ?? []).length > 0 && (
                <div>
                  <span className="text-text-muted">弟子：</span>
                ...
                return (
                  <div className="pt-1 border-t border-border-primary mt-1">
                    {bal && bal.totalBalance > 0 && (
                      <div>
                        <span className="text-interactive font-medium">{fmtMoney(bal.totalBalance)} 余额</span>
                      </div>
                    )}
                    <span className="text-status-success">{fmtMoney(rev.totalIncome)} 入账</span>
                    <span className="text-text-muted mx-1">/</span>
                    <span className="text-status-error">{fmtMoney(rev.totalExpenses)} 支出</span>
                    <span className="text-text-muted mx-1">/</span>
                    <span className={rev.netProfit >= 0 ? 'text-status-success font-medium' : 'text-status-error font-medium'}>
                      {rev.netProfit >= 0 ? '+' : ''}{fmtMoney(rev.netProfit)} 净余
                    </span>
                  </div>
                )
              })()}
