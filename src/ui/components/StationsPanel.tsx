  ...
  return (
    <div className="p-4 space-y-3">
              ...
              <div className="flex items-center gap-2">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-text-muted flex gap-2 mt-0.5">
                    ...
                    <span>{TIER_COSTS[station.tier] ?? `$${station.monthlyCost}/月`}</span>
                  </div>
                </div>
                <div className="text-xs text-text-muted text-right">
                  {station.status === 'canceling' && station.currentPeriodEnd
                    ? <span className="text-brand-700">截至 {formatShortDate(station.currentPeriodEnd)}</span>
                    : station.currentPeriodEnd
                      ? <>下次 {formatShortDate(station.currentPeriodEnd)}</>
                  ...
                  confirmAction?.id === station.id ? (
                    <div>
                      {confirmAction.action === 'cancel' && station.currentPeriodEnd && (
                        <div className="text-xs text-text-muted mb-1">Active 截至 {formatShortDate(station.currentPeriodEnd)}</div>
                      )}
      ...
      {payments && payments.length > 0 && (
        <div className="mt-4">
          <h3 className="text-sm font-medium text-text-secondary mb-2">钱庄支付记录</h3>
          <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
            {payments.map(p => (
