...
export function isLanHost(hostname = typeof location === 'undefined' ? '' : location.hostname): boolean {
...
export function getApiBase(): string {
  ...
  // On localhost or LAN direct access — use same-origin URLs.
...
async function fetchHandshakeToken(): Promise<string> {
  ...
  if (APP_MODE === 'local' && (!lastError || lastError === 'Failed to fetch')) {
    throw new Error(`无法连接本地服务端口 ${port}。请运行 "江湖 serve" 后重试。`)
  }
...
function getCloudControlOrigin(): string {
  return (import.meta.env.VITE_CLOUD_CONTROL_ORIGIN || 'http://127.0.0.1:4700').replace(/\/+$/, '')
}
