...
export const api = {
    ...
    status: () =>
      request<{
        ...
        apiAuth: {
          ...
          mimo: { hasRoomCredential: boolean; hasSavedKey: boolean; hasEnvKey: boolean; ready: boolean; maskedKey: string | null }
        }
      }>('GET', '/api/clerk/status'),
    setApiKey: (provider: 'openai_api' | 'anthropic_api' | 'gemini_api' | 'mimo_api', key: string) =>
      request<{
        ok: true
        apiAuth: {
          openai: { hasRoomCredential: boolean; hasSavedKey: boolean; hasEnvKey: boolean; ready: boolean; maskedKey: string | null }
  ...
  wallet: {
    ...
    record: (roomId: number, data: { direction: 'income' | 'expense'; amount: string; category?: string; counterparty?: string; description?: string }) =>
      request<WalletTransaction>('POST', `/api/rooms/${roomId}/wallet/transactions`, data),
