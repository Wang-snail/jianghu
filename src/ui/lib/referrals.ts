...
const LOCAL_INVITE_ORIGIN = 'http://127.0.0.1:4700'
...
export function buildKeeperInviteLink(code: string, origin = LOCAL_INVITE_ORIGIN): string {
  const normalized = normalizeReferralCode(code)
  if (!normalized) return origin
  return `${origin}/invite/${encodeURIComponent(normalized)}`
}

export function buildKeeperShareLink(code: string, shareId = 'v2', origin = LOCAL_INVITE_ORIGIN): string {
  const normalized = normalizeReferralCode(code)
