...
function tryRestartServer(): void {
  // Read the token from ~/.company/token to authenticate with the server
  const dataDir = path.join(homedir(), '.company')
  ...
  if (!fs.existsSync(tokenPath) || !fs.existsSync(portPath)) {
    console.log('No running server detected. Start with: company serve')
    return
  }
