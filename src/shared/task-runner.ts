...
function keeperReferralContext(db: Database.Database): string | null {
  ...
  return `## 本地推荐信息
- 用户代码：${code}
- 本地邀请：company-local://invite/${encoded}
- 本地分享：company-local://share/${encoded}`
}
...
export async function executeTask(
  ...
  const isStationModel = model?.startsWith('mimo:') || model?.startsWith('openai:') || model?.startsWith('anthropic:') || model?.startsWith('claude-api:')
