...
export async function executeAgent(options: AgentExecutionOptions): Promise<AgentExecutionResult> {
  ...
  if (model.startsWith('ollama:')) {
    throw new Error(`Ollama models are no longer supported. Update your company model to 'mimo:*', 'claude', 'codex', 'anthropic:*', or 'openai:*'.`)
  }
  if (model === 'codex' || model.startsWith('codex:')) {
    return executeCodex(options)
  }
  if (model === 'openai' || model.startsWith('openai:') || model === 'gemini' || model.startsWith('gemini:') || model === 'mimo' || model.startsWith('mimo:')) {
  ...
  throw new Error(`Unsupported model "${model}". Configure an explicit supported model (mimo:*, claude, codex, openai:*, anthropic:*, gemini:*).`)
}
...
// ─── OpenAI-compatible config resolver (OpenAI + Gemini + MiMo) ──────────
...
function resolveOpenAiCompatible(model: string, apiKeyOverride?: string): OpenAiCompatibleConfig | null {
  ...
  if (trimmed === 'mimo' || trimmed.startsWith('mimo:')) {
    const apiKey = apiKeyOverride?.trim() || (process.env.MIMO_API_KEY || '').trim()
    if (!apiKey) return null
...
async function executeOpenAiWithTools(options: AgentExecutionOptions): Promise<AgentExecutionResult> {
  ...
  if (!config) return immediateError(`Missing ${options.model.startsWith('gemini') ? 'Gemini' : options.model.startsWith('mimo') ? 'MiMo' : 'OpenAI'} API key.`)
...
function getAnthropicApiKey(apiKeyOverride?: string): string {
  return apiKeyOverride?.trim()
    || (process.env.ANTHROPIC_API_KEY || "").trim()
    || (process.env.ANTHROPIC_AUTH_TOKEN || "").trim()
}

function getAnthropicMessagesUrl(): string {
  let base = (process.env.ANTHROPIC_BASE_URL || "https://api.anthropic.com").trim()
...
async function executeAnthropicWithTools(options: AgentExecutionOptions): Promise<AgentExecutionResult> {
  const apiKey = getAnthropicApiKey(options.apiKey)
...
async function executeOpenAiApi(options: AgentExecutionOptions): Promise<AgentExecutionResult> {
  ...
  if (!config) {
    ...
    const isMimo = options.model.startsWith('mimo')
    const label = isGemini ? 'Gemini' : isMimo ? 'MiMo' : 'OpenAI'
    const credential = isGemini ? 'gemini_api_key' : isMimo ? 'mimo_api_key' : 'openai_api_key'
    const envVar = isGemini ? 'GEMINI_API_KEY' : isMimo ? 'MIMO_API_KEY' : 'OPENAI_API_KEY'
    return immediateError(`Missing ${label} API key. Set company credential "${credential}" or ${envVar}.`)
  }
...
function parseAnthropicModel(model: string): string {
  ...
  if (normalized === 'anthropic') return 'glm-4.7'
...
Respond ONLY with a JSON object (no markdown, no explanation):
{
  ...
  try {
    ...
    if (model === 'anthropic' || model.startsWith('anthropic:') || model.startsWith('claude-api:')) {
      const key = getAnthropicApiKey(apiKey)
      if (!key) return null
...
export async function executeApiOnStation(
  ...
  const isOpenAiCompat = options.model.startsWith('openai:') || options.model.startsWith('gemini:') || options.model.startsWith('mimo:')
