/**
 * Optional remote sync — disabled by default in local company mode.
 *
 * When public_mode is explicitly enabled, the engine:
 * 1. Registers with POST /api/rooms/register
 * 2. Sends heartbeats every 5 minutes with room stats + mode badge
 *
 * All requests fail silently, and remote availability never affects local operation.
 */
...
function getCloudTokenFilePath(): string {
  const explicitDataDir = process.env.COMPANY_DATA_DIR?.trim()
  if (explicitDataDir) return join(explicitDataDir, TOKEN_FILE_NAME)

  const dbPath = process.env.COMPANY_DB_PATH?.trim()
  if (dbPath) return join(dirname(dbPath), TOKEN_FILE_NAME)

  return join(homedir(), '.company-local', TOKEN_FILE_NAME)
}
