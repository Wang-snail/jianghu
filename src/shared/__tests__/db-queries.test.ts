...
describe('getDueOnceTasks', () => {
  ...
  it('does not return future one-time tasks', () => {
    // Use local time to match SQLite's datetime('now', 'localtime')
    const now = new Date()
    const future = new Date(now.getTime() + 3600000) // 1 hour in future
    const futureLocal = new Date(future.getTime() - (now.getTimezoneOffset() * 60000))
      .toISOString()
      .replace('T', ' ')
      .replace('Z', '')
...
describe('credentials', () => {
  ...
  it('upserts credential by room and name', () => {
    const first = q.createCredential(db, roomId, 'openai_api_key', 'api_key', 'sk-old')
    const second = q.createCredential(db, roomId, 'openai_api_key', 'api_key', 'sk-new')
    const list = q.listCredentials(db, roomId)
    expect(list).toHaveLength(1)
    expect(first.id).toBe(second.id)
    const full = q.getCredentialByName(db, roomId, 'openai_api_key')
    expect(full?.valueEncrypted).toBe('sk-new')
  })
})
...
describe('pickQueenNickname', () => {
  it('returns a non-empty string', () => {
    const name = q.pickQueenNickname(db)
    expect(typeof name).toBe('string')
    expect(name.length).toBeGreaterThan(0)
  })

  it('avoids names already used by other rooms', () => {
    // create rooms until only one name is left unused
    // Instead: fill up all-but-one names by direct insert
    const allNames: string[] = []
    // collect ~5 names by calling pickQueenNickname repeatedly on a fresh db,
    // then ensure the next pick avoids them
    const name1 = q.pickQueenNickname(db)
    q.createRoom(db, 'R1', undefined, undefined, undefined, name1)
    const name2 = q.pickQueenNickname(db)
  ...
  it('recycles names when all default gang leader nicknames are taken', () => {
    ...
    // Should still return a name (falls back to full pool)
    const name = q.pickQueenNickname(db)
    expect(typeof name).toBe('string')
    expect(name.length).toBeGreaterThan(0)
  })
...
describe('createRoom queenNickname', () => {
  it('auto-generates a queenNickname when not provided', () => {
    const room = q.createRoom(db, 'AutoNick Room')
    expect(typeof room.queenNickname).toBe('string')
    expect(room.queenNickname!.length).toBeGreaterThan(0)
  })

  it('uses provided queenNickname when given', () => {
    const room = q.createRoom(db, 'Named Room', undefined, undefined, undefined, 'Kira')
    expect(room.queenNickname).toBe('Kira')
  })

  it('assigns unique nicknames to consecutive rooms', () => {
    const r1 = q.createRoom(db, 'Room A')
    const r2 = q.createRoom(db, 'Room B')
    expect(r1.queenNickname).not.toBe(r2.queenNickname)
  })
})

describe('updateRoom queenNickname', () => {
  it('updates queenNickname', () => {
    const room = q.createRoom(db, 'NickUpdate')
    q.updateRoom(db, room.id, { queenNickname: 'Zara' })
    expect(q.getRoom(db, room.id)!.queenNickname).toBe('Zara')
  })

  it('leaves other fields unchanged when updating queenNickname', () => {
    const room = q.createRoom(db, 'SideEffect', 'my goal')
    q.updateRoom(db, room.id, { queenNickname: 'Luna' })
    const updated = q.getRoom(db, room.id)!
    expect(updated.name).toBe('SideEffect')
