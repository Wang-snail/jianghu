...
import { CLERK_TOOL_DEFINITIONS, executeClerkTool, selectClerkHermesForMessage } from '../clerk-tools'
...
describe('executeClerkTool', () => {
  it('creates room from objective only and auto-generates room name', async () => {
    const created = await executeClerkTool(db, 'company_create_room', {
  ...
  it('requires objective when creating a room', async () => {
    const created = await executeClerkTool(db, 'company_create_room', { name: 'emptygoal' })
  ...
  it('creates, updates, pauses, restarts, and deletes rooms', async () => {
    ...
    const updated = await executeClerkTool(db, 'company_update_room', {
    ...
    const paused = await executeClerkTool(db, 'company_pause_room', { roomId: room!.id })
    expect(paused.isError).toBeFalsy()
    expect(queries.getRoom(db, room!.id)!.status).toBe('paused')

    const restarted = await executeClerkTool(db, 'company_restart_room', { roomId: room!.id, goal: 'new mission' })
    ...
    const deleted = await executeClerkTool(db, 'company_delete_room', { roomId: room!.id })
    expect(deleted.isError).toBeFalsy()
    expect(queries.getRoom(db, room!.id)).toBeNull()
  })

  it('reads and writes global settings', async () => {
    const setResult = await executeClerkTool(db, 'company_set_setting', {
    ...
    const getResult = await executeClerkTool(db, 'company_get_setting', {
      key: 'clerk_commentary_enabled',
    })
  ...
  it('lists rooms and returns errors for unknown room targets', async () => {
    ...
    const listed = await executeClerkTool(db, 'company_list_rooms', {})
    expect(listed.isError).toBeFalsy()
    expect(listed.content).toContain('alpha')

    const missing = await executeClerkTool(db, 'company_pause_room', { roomName: 'does-not-exist' })
  ...
  it('sends keeper messages to a local room', async () => {
    ...
    const sent = await executeClerkTool(db, 'company_message_room', {
  ...
  it('queues inter-room message to another local room by room name', async () => {
    ...
    const queued = await executeClerkTool(db, 'company_message_other_room', {
  ...
  it('creates tasks and schedules keeper reminders', async () => {
    ...
    const createdTask = await executeClerkTool(db, 'company_create_task', {
      name: 'Daily brief',
    ...
    const reminder = await executeClerkTool(db, 'company_remind_keeper', {
...
describe('selectClerkHermesForMessage', () => {
  it('only wakes a small read-only Hermes set for status questions', () => {
    const selected = selectClerkHermesForMessage('帮我看看现在江湖里有哪些帮派和镖单在运行')
    const toolNames = selected.toolDefs.map((tool) => tool.function.name)

    expect(selected.profiles.map((profile) => profile.name)).toContain('巡江使')
    expect(selected.toolDefs.length).toBeLessThan(CLERK_TOOL_DEFINITIONS.length)
    expect(toolNames).toContain('company_list_rooms')
    expect(toolNames).toContain('company_list_tasks')
    expect(toolNames).not.toContain('company_delete_room')
    expect(toolNames).not.toContain('company_set_setting')
  })

  it('wakes the gatekeeper Hermes only for destructive control requests', () => {
    const selected = selectClerkHermesForMessage('删除那个拥堵费帮派，并停止相关运行')
    const toolNames = selected.toolDefs.map((tool) => tool.function.name)
