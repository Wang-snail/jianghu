...
describe('wallet transactions', () => {
  ...
  it('records salary as an internal bank expense', () => {
    const room = queries.createRoom(db, 'Payroll Room')
    ...
    queries.createWallet(db, room.id, '0xPAYROLL', 'enc', 'base')

    const tx = queries.logInternalBankTransaction(db, room.id, {
      direction: 'expense',
      amount: '88.00',
      category: 'salary',
      counterparty: `员工：${worker.name}`,
      description: '按解决问题和目标进展发放工资',
    })

    const summary = queries.getRevenueSummary(db, room.id)
    expect(tx.category).toBe('salary')
    expect(tx.type).toBe('send')
    expect(summary.totalExpenses).toBe(88)
    expect(summary.netProfit).toBe(-88)
  })
})
