...
describe('buildRegistrationURI', () => {
  ...
  it('includes company metadata', () => {
    ...
    expect(json['x-company']).toBeDefined()
    expect(json['x-company'].architecture).toBe('collective')
    expect(json['x-company'].workerCount).toBe(0)
    expect(json['x-company'].visibility).toBe('private')
    expect(json['x-company'].threshold).toBe('majority')
  })
  ...
  it('reflects queen and workers', () => {
    ...
    expect(json['x-company'].queen).toBe('Regina')
    expect(json['x-company'].workerCount).toBe(2) // Scout + Regina
  })
  ...
  it('uses fallback description when room has no goal', () => {
    ...
    const base64 = uri.replace('data:application/json;base64,', '')
    const json = JSON.parse(Buffer.from(base64, 'base64').toString('utf8'))
    expect(json.description).toBe('帮派：No Goal Room')
  })
