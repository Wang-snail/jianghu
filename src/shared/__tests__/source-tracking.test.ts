...
describe('task source tracking', () => {
  ...
  it('stores company UI source in triggerConfig', () => {
    const task = queries.createTask(db, {
      name: 'UI Task',
      prompt: 'Do something',
      triggerType: 'manual',
      triggerConfig: JSON.stringify({ source: 'company' })
    })

    const fetched = queries.getTask(db, task.id)
    const config = JSON.parse(fetched!.triggerConfig!)
    expect(config.source).toBe('company')
  })
...
describe('task creation modes', () => {
  ...
  it('creates one-time task with source', () => {
    ...
    expect(JSON.parse(fetched!.triggerConfig!).source).toBe('company')
  })
})
