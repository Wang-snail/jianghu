...
export function getAnnouncedDecisions(db: Database.Database): QuorumDecision[] {
  const rows = db.prepare("SELECT * FROM quorum_decisions WHERE status = 'announced' AND effective_at IS NOT NULL AND ((instr(effective_at, 'T') > 0 AND datetime(effective_at) <= datetime('now')) OR (instr(effective_at, 'T') = 0 AND datetime(effective_at) <= datetime('now','localtime')))").all()
...
export function getExpiredDecisions(db: Database.Database): QuorumDecision[] {
  const rows = db.prepare("SELECT * FROM quorum_decisions WHERE status = 'voting' AND timeout_at IS NOT NULL AND ((instr(timeout_at, 'T') > 0 AND datetime(timeout_at) <= datetime('now')) OR (instr(timeout_at, 'T') = 0 AND datetime(timeout_at) <= datetime('now','localtime')))").all()
  return (rows as Record<string, unknown>[]).map(mapDecisionRow)
}
...
export function getActiveSkillsForContext(db: Database.Database, roomId: number, contextText: string): Skill[] {
  ...
  function score(skill: Skill): number {
    if (!skill.activationContext || skill.activationContext.length === 0) return 1
    let matched = 0
    let longest = 0
    for (const keyword of skill.activationContext) {
      const normalized = keyword.toLowerCase()
      if (!normalized || !lower.includes(normalized)) continue
      matched += 1
      longest = Math.max(longest, normalized.length)
    }
    return matched === 0 ? 0 : 10 + matched * 4 + Math.min(longest, 40)
  }
  return mapped
...
export function logInternalBankTransaction(
  db: Database.Database,
  roomId: number,
  input: {
  ...
  return logWalletTransaction(
    db,
    wallet.id,
    input.direction === 'income' ? 'receive' : 'send',
    parsed.toFixed(2),
    {
...
function clerkApiKeySetting(provider: ClerkApiProvider): string {
  ...
  if (provider === 'mimo_api') return 'clerk_mimo_api_key'
