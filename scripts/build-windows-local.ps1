...
if (-not $OutFile) { $OutFile = "artifacts/windows/company-v$Version-win-local.exe" }
...
$stagingApp = Join-Path $stagingRoot "company"
...
$wrapperPath = Join-Path $stagingApp "bin/company.cmd"
Write-CrLfAsciiFile $wrapperPath @(
  ...
  "set ""USER_APP=%USERPROFILE%\.company\app""",
...
$zipOut = Join-Path $outDir "company-v$Version-win-local.zip"
if (Test-Path $zipOut) { Remove-Item -Force $zipOut }
Compress-Archive -Path $stagingApp -DestinationPath $zipOut -Force

$nsiPath = Join-Path $repoRoot "installers/windows/company.nsi"
& $makensis `
  "/DVERSION=$Version" `
