 ...
 *
 * Defaults:
 *   target-db: ~/.company/data.db
 *   room: null (global memories)
 *
 * Examples:
 *   node scripts/dump-memories.js /tmp/company-growth-XXX/data.db
 *   node scripts/dump-memories.js /tmp/company-growth-XXX/data.db --room 1
 *   node scripts/dump-memories.js /tmp/experiment.db ~/.company/data.db --room 2
 */
...
if (args.length === 0 || args.includes('--help')) {
  ...
  console.log('Default target: ~/.company/data.db, default room: null (global)')
  process.exit(0)
}

// Parse args
let srcPath = null
let dstPath = path.join(homedir(), '.company', 'data.db')
