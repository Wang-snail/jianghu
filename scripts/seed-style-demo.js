...
const ISOLATED_DIR_NAME = '.company-dev'
...
function resolveDbPath() {
  ...
  const dataDir = normalizePath(process.env.COMPANY_DATA_DIR || join(homedir(), ISOLATED_DIR_NAME))
...
if (!isIsolatedPath(dbPath)) {
  ...
  console.error(`Set COMPANY_DATA_DIR to a path under ~/${ISOLATED_DIR_NAME} (or use npm run seed:style-demo).`)
  process.exit(1)
}
