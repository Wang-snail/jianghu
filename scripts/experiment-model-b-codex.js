...
- Delegate execution to workers using company_delegate_task.
- Poke workers with company_send_message.
- Keep a short governance heartbeat with company_announce when appropriate.

Worker requirements:
- Execute delegated tasks with company_web_search / company_web_fetch.
- Store concrete findings with company_remember.
- Report progress and complete delegated goals.`

const QUEEN_SYSTEM_PROMPT = `You are the queen in Model B controller mode.

Rules:
1) If a non-queen worker exists, delegate one concrete task every cycle with company_delegate_task.
2) After delegating, send a direct poke/follow-up to that worker with company_send_message.
...
5) Always save cycle continuity with company_save_wip.
...
1) Execute immediately with company_web_search and optional company_web_fetch.
2) Store at least one concrete result using company_remember.
...
function startServer() {
  ...
  const ts = Date.now()
  dataDir = `/tmp/company-model-b-codex-${ts}`
...
async function main() {
  ...
  for (const row of allToolCalls) {
    ...
    if (row.workerId === queen.id) {
      if (toolName === 'company_delegate_task') queenDelegations++
      if (toolName === 'company_send_message') queenPokes++
