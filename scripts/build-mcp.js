...
async function main() {
  ...
  // In local dev, COMPANY_SKIP_MCP_NPM_INSTALL=1 can skip this intentionally.
  const skipInstall = process.env.COMPANY_SKIP_MCP_NPM_INSTALL === '1'
  const installReasons = getInstallReasons()
  if (installReasons.length > 0) {
    if (skipInstall) {
      console.warn(
        'Skipping out/mcp npm install due COMPANY_SKIP_MCP_NPM_INSTALL=1 even though native deps look stale:\n'
