...
const COMMAND_FRAGMENTS = [
  '/usr/local/lib/company/lib/server.js',
  '/usr/local/lib/company/lib/cli.js serve',
  '/usr/local/lib/company/lib/cli.js mcp',
...
function mainUnix() {
  ...
  if (targets.length === 0) {
    console.log('[kill-company-runtime] no matching processes')
    return
  }

  const pids = [...new Set(targets.map(t => t.pid))]
  console.log(`[kill-company-runtime] matched ${pids.length} process(es)`)
  for (const row of targets) {
    console.log(`  ${row.pid} ${row.command}`)
  }

  if (dryRun) {
    console.log('[kill-company-runtime] dry-run mode; no signals sent')
    return
  }
  ...
  if (stillAlive.length > 0) {
    console.error(`[kill-company-runtime] failed to terminate PID(s): ${stillAlive.join(', ')}`)
    process.exitCode = 1
    return
  }

  console.log('[kill-company-runtime] cleanup complete')
}

function main() {
  if (IS_WIN) {
    console.log('[kill-company-runtime] unsupported on Windows; skipping command-pattern cleanup')
    return
  }
  mainUnix()
}
