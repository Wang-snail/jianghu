...
const EXPERIMENT_TOOLS = [
  'company_set_goal', 'company_update_progress', 'company_complete_goal',
  'company_remember', 'company_recall',
  'company_send_message',
  'company_web_search', 'company_web_fetch',
].join(',')
...
How to earn:
1. Message the keeper (company_send_message to="keeper") to pitch your services and ask what they need
2. Check inbox for messages from other rooms (company_inbox_list) and respond to opportunities
3. When you get a contract, delegate tasks to workers and deliver results
4. Propose pricing decisions via company_propose so the team votes on rates

Coordination rules:
- Queen: delegate tasks to workers by name using company_delegate_task
- Workers: check "Your Assigned Tasks" and complete them, report progress
- Use company_send_message to coordinate with teammates
- Use company_propose for pricing, strategy, and major decisions — then everyone VOTE
- Use company_remember to store findings for the whole team
...
function build() {
  ...
  // (Claude CLI reads MCP from /usr/local/lib/company/lib/server.js via ~/.claude.json)
...
// ─── MCP DB override ─────────────────────────────────────────────────────────
// Claude CLI reads MCP config from ~/.claude.json which hardcodes COMPANY_DB_PATH.
...
function startServer(env) {
  ...
  const ts = Date.now()
  dataDir = `/tmp/company-experiment-${ts}`
...
async function setupSwarm(token, apiKeys) {
  ...
  for (let i = 0; i < workerModels.length; i++) {
    ...
    const workerRes = await api('POST', '/api/workers', {
      name: m.label,
      systemPrompt: `You are a worker agent in a multi-agent swarm. Your name is "${m.label}". Your model is ${m.model}. Collaborate with other agents using company_send_message (specify the worker name in "to"). Vote on proposals with company_vote. Store findings in memory with company_remember so all agents can access them. Focus on making measurable progress each cycle.`,
      roomId: room.id,
