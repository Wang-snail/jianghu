...
const GOAL = `Research the top 3 AI agent frameworks (crew.ai, autogen, langgraph). For each one, use web_search to find their pricing and key features. Store each framework's details in memory using company_remember.

IMPORTANT: At the END of each cycle, save your progress with company_save_wip so the next cycle knows where you left off. Example: company_save_wip({ status: "Researched crew.ai and autogen. Next: research langgraph and write comparison." })`
...
function overrideMcpConfig(experimentDbPath) {
  try {
    ...
    if (existsSync(localServer)) {
      originalMcpArgs = [...(company.args || [])]
      company.args = [localServer]
...
function startServer() {
  ...
  const ts = Date.now()
  dataDir = `/tmp/company-wip-test-${ts}`
...
async function main() {
  ...
  const checks = [
    ...
    [wipUsedSaveWip, 'Agent called company_save_wip tool'],
