...
function getMcpDbPath(configPath) {
  ...
  return cfg?.mcpServers?.company?.env?.COMPANY_DB_PATH || null
}
...
function main() {
  ...
  if (claudeDbPath) {
    console.log(`company MCP DB: ${claudeDbPath}`)
  } else if (readJsonSafe(claudePath) === 'invalid_json') {
    console.log('company MCP DB: (invalid JSON)')
  } else {
    console.log('company MCP DB: (not set)')
  }
  ...
  if (claudeDbPath && claudeDbPath.includes('.company-dev')) {
    issues.push('MCP config points to .company-dev (dev DB), not release DB.')
  }
