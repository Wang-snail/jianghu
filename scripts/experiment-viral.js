...
1. GENERATE THE KEEPER'S INVITE LINK
   - Call company_invite_create to generate a referral link
   - Store it in memory with company_remember("invite_link", ...)
   - Share it with all workers via company_send_message
...
4. ACTUALLY POST (delegate to poster-a and poster-b)
   - Use company_browser to navigate to found threads and post comments
   - Adapt the comment to fit the specific thread context
   - Record every post: platform, URL, comment text, result

COORDINATION RULES:
- Queen: generate invite link FIRST, then delegate. Track progress with company_update_progress.
- Workers: check company_recall for the invite link before starting. Check company_list_skills for platform-specific posting algorithms.
- Every worker: message company_send_message(to="keeper") with progress updates.
...
const TOOLS = [
  ...
  'company_invite_create', 'company_invite_list',
].join(',')
...
Also check company_recall for the invite link — you'll need to include it in your scouting report.
...
First: check company_recall for the invite link — include it in every comment draft.
Also check company_recall for scout's thread findings — tailor comments to those specific contexts.

Draft 5-6 comment variations:
1. Technical angle: "I've been building with company — the multi-agent voting system is interesting because..."
2. Builder angle: "Just launched an open-source platform for autonomous agent teams — company.ai — would love feedback from this community"
3. Curious angle: "Has anyone tried setting up rooms where agents vote on decisions? We built..."
4. Revenue angle: "The interesting thing about company is agents can actually earn USDC..."
...
Store all drafts with company_remember("writer: comment drafts", ...) so posters can copy-paste and adapt.`
  },
  {
    name: 'poster-a',
    role: 'Reddit Poster',
...
Before starting:
- company_recall for the invite link
- company_recall for scout's Reddit thread findings
- company_recall for writer's comment drafts
- company_list_skills for any Reddit login or posting algorithms

Use company_browser to:
1. Navigate to Reddit threads found by scout (or find new ones with company_web_search)
2. Log in to Reddit (you may need to create an account first — use company_browser to sign up)
...
- Store credentials with company_remember("poster-a: reddit credentials", ...)
- Handle any CAPTCHA with screenshot action → Read tool to view image

For each post attempt — success OR failure:
- Store result with company_remember("poster-a: post [thread]", result)
...
Before starting:
- company_recall for the invite link
- company_recall for scout's dev.to / HN findings
- company_recall for writer's comment drafts
- company_list_skills for any existing login algorithms
...
HACKER NEWS:
- Find relevant "Show HN" or "Ask HN" threads via company_web_search
...
For each post:
- Store result with company_remember("poster-b: post [platform] [thread]", ...)
...
- Check company_list_skills — another agent may have already documented what you need
- Check company_recall for the invite link, scout findings, and writer's comment drafts
- Don't repeat work that's already done

Store results immediately:
- Use company_remember("${r.name}: [what]", ...) so teammates can find your work
- Message teammates with company_send_message when you have something they need

MANDATORY — Execution Report (every cycle):
Before your cycle ends, call company_create_skill with a report:
...
function printResults(swarm) {
  ...
  if (invites.length > 0) {
    ...
    for (const inv of invites) {
      console.log(`  company.ai/join/${inv.code}  (used: ${inv.used_count})`)
    }
  }
